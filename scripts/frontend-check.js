...
const checks = [
  ...
  ["录音与文件选择盒统一对齐且窄屏无表单外边距", /\.voice-file-label\s*\{[^}]*margin:\s*0\s*;/s.test(voiceCss) && voiceCss.includes(".voice-actions .button,") && voiceCss.includes(".voice-actions .voice-file-label,")],
  ...
  ["今日回访懒加载且始终只呈现一件", revisitsApp.includes("if (!explicitKind && loaded && !busyLoad) return current;") && revisitsApp.includes("const candidates = explicitKind ? [explicitKind] : KINDS") && revisitsApp.includes("revisit: revisits[0]") === false && revisitsApp.includes("current = revisit") && app.includes('if (target === "reflect")')],
