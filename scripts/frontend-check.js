...
const multiPerspectiveCss = read("public/multi-perspective.css");
const semanticRecallCss = read("public/semantic-recall.css");
const app = read("public/assets/app.js");
const lazyFeaturesApp = read("public/assets/lazy-features.js");
...
const multiPerspectiveApp = read("public/assets/multi-perspective.js");
const multiPerspectiveHostApp = read("public/assets/multi-perspective-host.js");
const semanticRecallApp = read("public/assets/semantic-recall.js");
const semanticRecallWorker = read("public/assets/semantic-recall-worker.js");
const semanticRecallApi = read("lib/semantic-recall-api.js");
const semanticRecallService = read("lib/semantic-recall-service.js");
const memoryLensApi = read("lib/memory-lens-api.js");
const multiPerspectiveApi = read("lib/multi-perspective-api.js");
const multiPerspectiveService = read("lib/multi-perspective-service.js");
...
const isolatedRecoveryDrill = read("lib/isolated-recovery-drill.js");
const isolatedRecoveryApi = read("lib/isolated-recovery-api.js");
const isolatedRecoveryRuntime = read("lib/isolated-recovery-runtime.js");
const server = read("server.js");
const runtimeTrust = read("lib/runtime-trust.js");
const aiTrust = read("lib/ai-trust.js");
...
const queriedIds = [app, pwaApp, mediaApp, voiceApp, capsuleApp, sharePrivacyApp, mediaEvidenceApp, portabilityApp, exhibitionsApp, curatorAgentApp, revisitsApp, cluesApp, memoryInboxApp, provenanceApp, museumLockApp, multiPerspectiveHostApp, semanticRecallApp].flatMap((source) => [
...
const runtimeOnlyIds = new Set(["agentTraceTitle"]);
...
const capsuleLazyTokens = [
  'lazyFeatures.loadScript("/assets/capsule-crypto.js"',
  'lazyFeatures.loadScript("/assets/share-privacy.js"',
  'lazyFeatures.loadScript("/assets/capsules.js"'
];
...
const checks = [
  ...
  ["PWA resources load in manifest-style-controller-app order", html.includes(`/manifest.webmanifest?v=${pkg.version}`) && html.indexOf('/styles.css') < html.indexOf(`/pwa.css?v=${pkg.version}`) && html.indexOf(`/assets/pwa.js?v=${pkg.version}`) < html.indexOf(`/assets/app.js?v=${pkg.version}`)],
  ...
  ["馆藏体检收进项目幕后且保持只读渐进披露", html.includes('id="collectionHealthDetails"') && html.indexOf('id="collectionHealthDetails"') > html.indexOf('id="exportButton"') && !html.includes('<details class="collection-health-panel" id="collectionHealthDetails" open') && collectionHealthApp.includes("/api/collection-health/scans") && collectionHealthApp.includes("/api/archive/inspect") && collectionHealthApp.includes("不会自动删除或改写") && !/localStorage|sessionStorage|indexedDB/iu.test(collectionHealthApp)],
  ["馆藏体检明确 Demo 与移动端安全边界", collectionHealthApp.includes("公开只读示例不提供本机馆藏体检") && collectionHealthApp.includes("不会恢复到当前馆藏") && collectionHealthCss.includes("@media (max-width: 650px)") && collectionHealthCss.includes("@media (max-width: 360px)") && !/gradient\s*\(/iu.test(collectionHealthCss) && app.includes("TimeIsleCollectionHealth?.createController")],
  ...
  ["Capsule scripts stay off the cold path and preserve dependency order", (html.match(/\/capsules\.css/g) || []).length === 1 && (html.match(/\/share-privacy\.css/g) || []).length === 1 && !html.includes('/assets/capsule-crypto.js') && !html.includes('/assets/share-privacy.js') && !html.includes('/assets/capsules.js') && capsuleLazyTokens.every((token) => app.includes(token)) && capsuleLazyTokens.every((token, index) => index === 0 || app.indexOf(capsuleLazyTokens[index - 1]) < app.indexOf(token))],
  ["Lazy feature loader precedes the app and retries incomplete resources", html.indexOf(`/assets/lazy-features.js?v=${pkg.version}`) > 0 && html.indexOf(`/assets/lazy-features.js?v=${pkg.version}`) < html.indexOf(`/assets/app.js?v=${pkg.version}`) && lazyFeaturesApp.includes("promises.delete(source)") && lazyFeaturesApp.includes("script.remove()") && lazyFeaturesApp.includes("资源加载超时") && lazyFeaturesApp.includes("资源加载不完整")],
  ...
  ["脚本查询的 DOM ID 全部存在且页面 ID 不重复", queriedIds.every((id) => htmlIds.includes(id) || runtimeOnlyIds.has(id)) && [...runtimeOnlyIds].every((id) => app.includes(`id="${id}"`)) && new Set(htmlIds).size === htmlIds.length],
  ...
  ["核心表单错误使用中文说明并聚焦首个无效字段", ["memoryForm", "draftForm", "guideForm"].every((id) => new RegExp(`<form[^>]*id="${id}"[^>]*novalidate`).test(html)) && app.includes("function validateForm(form, status)") && app.includes('field.setAttribute("aria-invalid", "true")') && app.includes('field.setAttribute("aria-describedby", status.id)') && app.includes('field.focus({ preventScroll: false })') && app.includes("clearFieldValidation(event.target")],
  ...
  ["V11 收件箱保持 Demo 零写和来源边界", server.includes("isInterviewDemoBlockedMutation(request, url)") && server.includes('!["GET", "HEAD"].includes') && server.indexOf("if (isInterviewDemoBlockedMutation(request, url))") < server.indexOf("museumWriteRuntime.enterHttpRequest") && memoryInboxApp.includes("elements.file.disabled = demo") && memoryInboxApp.includes("fileState.bytes.fill(0)")],
  ...
  ["V11.1 来源护照留在展品详情且默认折叠", !html.includes("/assets/provenance.js") && app.includes('lazyFeatures.loadScript("/assets/provenance.js"') && app.includes("TimeIsleProvenance?.renderPanel(memory)") && provenanceApp.includes('<details class="provenance-passport"') && !provenanceApp.includes('<details class="provenance-passport" open')],
  ...
  ["V12 共忆资源按 crypto-letter-host-app 顺序接入", ["co-memory-crypto", "co-memory-letter", "co-memory-host"].every((name) => !html.includes("/assets/" + name + ".js")) && app.indexOf('loadScript("/assets/co-memory-crypto.js"') < app.indexOf('loadScript("/assets/co-memory-letter.js"') && app.indexOf('loadScript("/assets/co-memory-letter.js"') < app.indexOf('loadScript("/assets/co-memory-host.js"') && html.includes("/co-memory-letter.css")],
  ...
  ["V15 真实隔离恢复仍在默认折叠的数据面板且结构预检降为兼容详情", html.includes('id="isolatedRecoveryFile"') && html.includes('id="isolatedRecoverySteps"') && html.includes('class="structural-recovery-legacy"') && !html.includes('<details class="structural-recovery-legacy" open') && html.indexOf('id="isolatedRecoveryFile"') < html.indexOf('id="structuralRecoveryFile"') && (html.match(/class="nav-button/g) || []).length === 4],
  ["V15 客户端只接受已销毁、无当前馆藏能力且不夸大灾备的安全回执", museumLockApp.includes('requestJson("/api/recovery-drills/isolated-restore"') && museumLockApp.includes('receipt?.kind !== "isolated-restore"') && museumLockApp.includes('currentMuseumCapabilityProvided !== false') && museumLockApp.includes('sandboxDestroyed !== true') && museumLockApp.includes('disasterRecoveryProven !== false') && !/localStorage|sessionStorage|indexedDB/iu.test(museumLockApp)],
  ["V15 服务端真实恢复到 fresh store/storage 并在关闭销毁后才通过", isolatedRecoveryRuntime.includes("createStore({ dbPath, halls, schemaVersion })") && isolatedRecoveryRuntime.includes("restoreArchive({") && isolatedRecoveryRuntime.includes("runDatabaseHealthChecks()") && isolatedRecoveryRuntime.includes("verifyVariant(variant") && isolatedRecoveryRuntime.includes("voiceStorage.verify(asset") && isolatedRecoveryRuntime.includes("sandbox.store.close()") && isolatedRecoveryRuntime.includes("removeSandboxRoot(sandbox.root)") && isolatedRecoveryDrill.indexOf("destroySandbox") < isolatedRecoveryDrill.indexOf('verdict: "passed-isolated-restore"')],
  ["V15 API 公开 Demo 正文前拒绝且安全投影不返回内容、ID、路径或单文件哈希", isolatedRecoveryApi.includes("bodyBytesRead: 0") && isolatedRecoveryApi.includes("Within this route handler") && isolatedRecoveryApi.includes("global Host/Origin validation") && isolatedRecoveryApi.includes("publicIsolatedRecoveryReceipt") && !/(?:privateTitle|privatePath|idMap|archiveFingerprint|sha256\s*:)/u.test(isolatedRecoveryApi)],
  ["V15 一次性恢复面板保持三档移动端触控与无渐变", museumLockCss.includes(".isolated-recovery .button") && museumLockCss.includes(".isolated-recovery-steps") && museumLockCss.includes("min-height: 44px") && museumLockCss.includes("@media (max-width: 650px)") && museumLockCss.includes("@media (max-width: 390px)") && museumLockCss.includes("@media (max-width: 320px)") && !/gradient\s*\(/iu.test(museumLockCss)],
  ["V16 多视角对照留在展品详情并默认折叠", (html.match(/class="nav-button/g) || []).length === 4 && app.includes("TimeIsleMultiPerspective?.renderPanel(memory)") && app.includes("multiPerspectiveController?.open(memory") && app.includes("multiPerspectiveController?.close()") && multiPerspectiveApp.includes('<details class="multi-perspective-panel"') && !multiPerspectiveApp.includes('<details class="multi-perspective-panel" open') && !html.includes('data-view="multi-perspective"')],
  ["V16 多视角资源按 UI-host-app 顺序接入当前版本", html.includes("/multi-perspective.css") && !html.includes("/assets/multi-perspective.js") && !html.includes("/assets/multi-perspective-host.js") && app.indexOf('loadScript("/assets/multi-perspective.js"') < app.indexOf('loadScript("/assets/multi-perspective-host.js"') && app.includes("TimeIsleMultiPerspectiveHost?.createController")],
  ["V16 对照 API 只按 GET 从本机已保存记录派生且 Demo 先于 store", multiPerspectiveApi.includes('request.method !== "GET"') && multiPerspectiveApi.includes("assertNoQuery") && multiPerspectiveApi.indexOf("? buildSyntheticMultiPerspectivePreview") < multiPerspectiveApi.indexOf(": buildStoredMultiPerspectivePreview") && server.includes("multiPerspectiveApi.handle") && multiPerspectiveService.includes("modelCalls: 0") && multiPerspectiveService.includes("toolCalls: 0") && multiPerspectiveService.includes("persisted: false")],
  ["V16 只呈现人工确认关系并保留未核验与待复核边界", ["supports", "supplements", "different_record"].every((kind) => multiPerspectiveService.includes(`${kind}:`)) && multiPerspectiveService.includes('lifecycleStatus !== "confirmed"') && multiPerspectiveApp.includes("身份未核验") && multiPerspectiveApp.includes("需要重新核对") && multiPerspectiveApp.includes("不会自动保存或判断谁对谁错")],
  ["V16 动态对照使用安全 DOM、零持久化和移动触控布局", !multiPerspectiveApp.includes("innerHTML") && !/(?:localStorage|sessionStorage|indexedDB)/iu.test(`${multiPerspectiveApp}\n${multiPerspectiveHostApp}`) && multiPerspectiveHostApp.includes('method: "GET"') && !multiPerspectiveHostApp.includes("body: JSON.stringify") && multiPerspectiveCss.includes("min-height: 44px") && multiPerspectiveCss.includes("@media (max-width: 650px)") && multiPerspectiveCss.includes("@media (max-width: 390px)") && multiPerspectiveCss.includes("@media (max-width: 320px)") && !/gradient\s*\(/iu.test(multiPerspectiveCss)],
  ["V17.2 按意思找回留在单一回忆工作区且默认不展开", (html.match(/class="nav-button/g) || []).length === 4 && html.includes('<details class="semantic-recall-card" id="semanticRecallDetails" hidden>') && !html.includes('<details class="semantic-recall-card" id="semanticRecallDetails" open') && html.indexOf('id="semanticRecallDetails"') > html.indexOf('data-view-panel="reflect"') && !html.includes('data-view="semantic-recall"')],
  ["V17 资源以样式、控制器、Worker、主应用边界接入", html.includes(`/semantic-recall.css?v=${pkg.version}`) && html.indexOf(`/assets/semantic-recall.js?v=${pkg.version}`) < html.indexOf(`/assets/app.js?v=${pkg.version}`) && app.includes("TimeIsleSemanticRecall?.createController") && semanticRecallWorker.includes('/assets/semantic-recall-core.mjs') && semanticRecallWorker.includes('/assets/vendor/transformers-3.8.1/transformers.min.js')],
  ["V17 快照只读投影五类文字并设 500 件上限", semanticRecallApi.includes('request?.method !== "GET"') && semanticRecallApi.includes("store.listMemories()") && semanticRecallService.includes("memories: 500") && ["title", "exhibitText", "rawContent", "tags", "confirmedTranscripts"].every((field) => semanticRecallService.includes(field)) && !/readJsonBody|request\.body/iu.test(semanticRecallApi)],
  ["V17 真实 embedding 仅使用同源模型与 Worker 会话内存", semanticRecallWorker.includes('pipeline("feature-extraction"') && semanticRecallWorker.includes('dtype: "q8"') && semanticRecallWorker.includes("env.allowRemoteModels = false") && semanticRecallWorker.includes("env.useBrowserCache = false") && !/localStorage|sessionStorage|indexedDB|https?:\/\//iu.test(`${semanticRecallApp}\n${semanticRecallWorker}`)],
  ["V17 小白边界、取消清理与窄屏触控完整", html.includes("相似不等于事实、人物关系、情绪、真实性或概率判断") && semanticRecallApp.includes("AbortController") && semanticRecallApp.includes("prepareEpoch") && semanticRecallApp.includes("terminateWorker()") && semanticRecallCss.includes("min-height: 44px") && semanticRecallCss.includes("@media (max-width: 390px)") && semanticRecallCss.includes("@media (max-width: 320px)") && !/gradient\s*\(/iu.test(semanticRecallCss)],
  ["V17.1 四视图外持续展示真实信任状态", html.indexOf('id="trustBar"') < html.indexOf('id="mainContent"') && ["trustMode", "trustStorage", "trustAi", "trustExternal", "trustEncryption"].every((id) => html.includes(`id="${id}"`)) && app.includes('requestJson("/api/runtime/trust")') && app.includes("function renderTrustStatus") && runtimeTrust.includes('audience: interviewDemo ? "public-demo" : "private-local"') && runtimeTrust.includes("blockedBeforeBodyRead: interviewDemo") && runtimeTrust.includes("enabled: false")],
  ["V17.1 记录流程先保存原文再可选整理同一展品", html.indexOf('id="saveOriginalButton"') < html.indexOf('id="analyzeButton"') && html.includes("第一步只要求原文") && app.includes("state.pendingSaveMemoryId = saved.memory.id") && app.indexOf("state.pendingSaveMemoryId = saved.memory.id") < app.indexOf('runAttachmentControllers("saveToMemory", saved.memory.id)') && app.includes("请先点击“保存记忆”") && app.includes("expectedUpdatedAt: state.draft.updatedAt") && app.includes("organizeReceipt: { confirmed: true") && server.includes("verifyExecutionReceipt")],
  ["V17.1 外部 AI 逐次披露并要求内容绑定信任合同", ["organizeExternalDisclosure", "guideExternalDisclosure", "organizeExternalConsent", "guideExternalConsent"].every((id) => html.includes(`id="${id}"`)) && app.includes('buildExternalAiConsent("organize", rawContent)') && app.includes('buildExternalAiConsent("guide", question, controller.signal)') && app.includes("elements.organizeExternalConsent.checked = false") && app.includes("elements.guideExternalConsent.checked = false") && server.includes("resolveExplicitExternalAiConsent") && aiTrust.includes("consent.contractId !== runtimeTrust.contractId") && aiTrust.includes("consent.inputSha256 !== digestInput(feature, input)") && aiTrust.includes("consumedConsentNonces.has(consent.nonce)") && aiTrust.includes("AI_CONSENT_REPLAYED") && runtimeTrust.includes('inputFieldsSent: aiEnabled ? ["rawContent"]') && runtimeTrust.includes('"evidence.exhibitText"')],
  ["V17.2 移动核心旅程沿用四路由并显示三种找回方式", ["馆藏", "记录", "找回", "我的"].every((label) => html.includes(`aria-label="${label}"`)) && ["collection", "compose", "reflect", "data"].every((view) => html.includes(`data-view="${view}"`)) && html.includes("按名字、地点或原句找") && html.includes("按意思找") && html.includes("向馆藏提问") && html.includes("更多回看方式") && html.includes("项目幕后：完整安全合同")],
  ...
  ["声音模块缺失不会伪装成已连接", app.includes("function initializeVoiceController") && app.includes("声音模块未能加载，请刷新页面重试。") && app.includes("其他馆藏功能不受影响") && app.indexOf("initializeControllers(options, state.demo.interviewDemo)") < app.indexOf("setRuntimeStatus(state.trustError")],
  ["麦克风授权等待直接反馈并隔离过期会话", voiceApp.includes('Symbol("voice-permission")') && voiceApp.includes("取消授权等待") && voiceApp.includes("仍在等待麦克风授权") && voiceApp.includes("requestSession !== session") && voiceApp.includes("isInteractionBusy()") && voiceApp.includes("mutationBusy || loadBusy || externalBusy") && voiceApp.includes("recording === startedRecording") && voiceApp.includes('setAttribute("aria-busy"') && voiceApp.includes("cancelPermissionRequest(true)")],
  ["录音协商格式、179 秒边界、离场保护与 Demo 零麦克风写入", voiceApp.indexOf("if (demo || recording") < voiceApp.indexOf("getUserMedia({ audio: true") && voiceApp.includes('"audio/webm;codecs=opus"') && voiceApp.includes('"audio/mp4;codecs=mp4a.40.2"') && voiceApp.includes("Math.min(179_000") && voiceApp.includes('listen(documentRef, "visibilitychange"') && voiceApp.includes('listen(lifecycleTarget, "pagehide"') && voiceApp.includes('prepareForViewLeave("keep")') && voiceApp.includes("stopTracks(active.stream)") && !voiceApp.includes("stopTracks(finished.stream)") && voiceApp.includes("revokeObjectURL") && voiceApp.includes("elements.voiceFileInput.disabled = demo") && app.includes("recordingNeedsDecision") && app.includes('prepareForViewLeave?.(action)')],
  ...
  ["照片线索资源已按依赖顺序接入", html.indexOf('/media-evidence.css') > html.indexOf('/media.css') && !html.includes('/assets/media-evidence.js') && app.includes('lazyFeatures.loadScript("/assets/media-evidence.js"') && html.indexOf('/assets/media.js') < html.indexOf('/assets/app.js')],
  ...
  ["文字摘录依赖先于影像实验台载入", !html.includes('/assets/media-ocr.js') && !html.includes('/assets/media-lab.js') && app.indexOf('loadScript("/assets/media-ocr.js"') < app.indexOf('loadScript("/assets/media-lab.js"')],
  ...
  ["主题展览资源按独立模块和依赖顺序接入", (html.match(/\/exhibitions\.css/g) || []).length === 1 && (html.match(/\/assets\/exhibitions\.js/g) || []).length === 1 && html.indexOf("/styles.css") < html.indexOf("/exhibitions.css") && html.indexOf("/assets/exhibitions.js") < html.indexOf("/assets/app.js") && !html.includes("/assets/media-lab.js") && app.includes("TimeIsleExhibitions?.createController")],
  ...
  ["V10 策展助手样式独立且控制器按需载入", (html.match(/\/curator-agent\.css/g) || []).length === 1 && !html.includes("/assets/curator-agent.js") && html.indexOf("/exhibitions.css") < html.indexOf("/curator-agent.css") && app.includes('lazyFeatures.loadScript("/assets/curator-agent.js"') && app.includes("TimeIsleCuratorAgent?.createController")],
  ...
  ["今日回访只占找回页一张渐进卡片", (html.match(/id="revisitCard"/g) || []).length === 1 && html.indexOf('id="revisitCard"') > html.indexOf('id="guideTitle"') && html.indexOf('id="revisitCard"') < html.indexOf('id="moreRecallDetails"') && (html.match(/data-revisit-kind=/g) || []).length === 3 && !html.includes('data-view="revisit"')],
  ...
  ["今日回访 Demo 不写状态、真实 cursor 翻页且移动触控完整", revisitsApp.includes("if (!demo)") && revisitsApp.includes("nextCursorByKind") && revisitsApp.includes('limit: "1"') && revisitsApp.includes("payload?.revisits") && revisitsApp.includes("function setStatus(text, isError = false)") && revisitsCss.includes("min-height: 44px;") && ["safe-area-inset-right", "safe-area-inset-bottom", "safe-area-inset-left"].every((token) => revisitsCss.includes(token)) && revisitsCss.includes("@media (max-width: 360px)")],
  ["字段与线索资源独立并在主应用前载入", (html.match(/\/clues\.css/g) || []).length === 1 && (html.match(/\/assets\/clues\.js/g) || []).length === 1 && html.indexOf('/revisits.css') < html.indexOf('/clues.css') && html.indexOf('/assets/revisits.js') < html.indexOf('/assets/clues.js') && html.indexOf('/assets/clues.js') < html.indexOf('/assets/app.js') && app.includes("TimeIsleClues?.createEntityDialogController")],
  ...
  ["唯一搜索框保留线索元数据、服务端筛选分页与回看依据", (html.match(/id="searchInput"/g) || []).length === 1 && !html.includes("data-search-mode") && !app.includes("/api/search?mode") && app.includes('new URLSearchParams({ query, limit: "30", sort: elements.sortSelect.value })') && app.includes('params.set("cursor", cursor)') && app.includes("normalizeSearchResponse(payload)") && app.includes("state.searchResponse =") && app.includes("renderSearchEvidence(searchResult, state.searchResponse.engine)") && cluesApp.includes("matchedTerms") && cluesApp.includes("matchedFields") && cluesApp.includes("entityRefs")],
  ...
  ["口述史保留单段采集依赖并在首次需要时载入控制器", html.indexOf(`/assets/voice-capture.js?v=${pkg.version}`) < html.indexOf(`/assets/app.js?v=${pkg.version}`) && !html.includes(`/assets/oral-histories.js?v=${pkg.version}`) && app.includes('lazyFeatures.loadScript("/assets/oral-histories.js"') && html.indexOf('/voice.css') < html.indexOf(`/oral-histories.css?v=${pkg.version}`)],
  ...
  ["详情异步失败会明确提示而不是静默无响应", app.includes("void openMemory(memoryId, target).catch") && app.includes('console.error("打开展品详情失败：", error)') && app.includes('showToast(`无法打开这件展品：${error.message}`, true)')],
  ...
  ["检索结果计数、零结果与失败通过状态区播报", /id="collectionMeta"[^>]*role="status"[^>]*aria-live="polite"[^>]*aria-atomic="true"/.test(html) && app.includes('`“${query}”找到 ${state.searchTotal} 件展品，已显示 ${visible.length} 件${filterNote}${') && app.includes("shortQueryFallback") && app.includes('elements.collectionMeta.textContent = "检索失败，请重试。";')],
  ...
  ["考古概览失败不会拖垮核心馆藏", app.includes("function loadArchaeologyAfterBootstrap") && app.includes('requestJson("/api/archaeology/overview")') && app.includes('.catch(() => ({ overview: [] }))')],
  ...
  ["附件关联失败后原文保留且复用已创建展品", app.includes("const targetMemoryId = state.editingMemoryId || state.pendingSaveMemoryId;") && app.includes('method: targetMemoryId ? "PUT" : "POST"') && app.indexOf("state.pendingSaveMemoryId = saved.memory.id") < app.indexOf('runAttachmentControllers("saveToMemory", saved.memory.id)') && app.includes("原文已安全保存") && app.includes("不会重复创建展品") && app.includes("修正附件后再次点击") && app.includes('state.pendingSaveMemoryId = "";')],
  ["后台媒体维护为尚未关联的新附件保留安全年龄窗口", server.includes('mediaApi.garbageCollect({ status: "ready", before, limit: 500 })') && server.includes('voiceApi.garbageCollect({ status: "ready", before, limit: 500 })')],
  ...
  [
    ...
    JSON.stringify(Object.keys(pkg.scripts || {}).sort()) === JSON.stringify([
      ...
      "test:browser",
      "test:semantic-eval"
    ]) && pkg.scripts["release:verify"] === "node scripts/release-evidence-check.js" &&
      pkg.scripts["test:browser"] === "npm --prefix tests/browser test" &&
      pkg.scripts["test:semantic-eval"] === "npm --prefix tests/browser test -- semantic-recall-eval.spec.cjs --project=desktop-1265"
  ],
  ["核心文件规模保持受控", lineCount(server) < 1800 && lineCount(app) < 2800 && lineCount(runtimeTrust) < 200 && lineCount(memoryInboxApp) < 550 && lineCount(provenanceApp) < 550 && lineCount(coMemoryLetterApp) < 950 && lineCount(coMemoryHostApp) < 120 && lineCount(memoryLensApp) < 1000 && lineCount(memoryLensHostApp) < 350 && lineCount(multiPerspectiveApp) < 600 && lineCount(multiPerspectiveHostApp) < 120 && lineCount(semanticRecallApp) < 500 && lineCount(semanticRecallWorker) < 240 && lineCount(museumLockApp) < 350 && lineCount(pwaApp) < 250 && lineCount(mediaApp) < 1200 && lineCount(voiceApp) < 1000 && lineCount(voiceCaptureApp) < 650 && lineCount(oralHistoryApp) < 1200 && lineCount(capsuleApp) < 1000 && lineCount(sharePrivacyApp) < 600 && lineCount(mediaEvidenceApp) < 850 && lineCount(mediaCompareApp) < 850 && lineCount(mediaOcrApp) < 750 && lineCount(mediaLabApp) < 500 && lineCount(portabilityApp) < 250 && lineCount(exhibitionsApp) < 850 && lineCount(revisitsApp) < 550 && lineCount(revisionsApp) < 400 && lineCount(cluesApp) < 750 && lineCount(collectionHealthApp) < 350 && lineCount(timeCalibrationApp) < 850 && lineCount(css) < 2350 && lineCount(memoryInboxCss) < 150 && lineCount(provenanceCss) < 300 && lineCount(coMemoryLetterCss) < 400 && lineCount(memoryLensCss) < 550 && lineCount(multiPerspectiveCss) < 400 && lineCount(semanticRecallCss) < 220 && lineCount(museumLockCss) < 250 && lineCount(pwaCss) < 250 && lineCount(archaeologyCss) < 400 && lineCount(mediaCss) < 700 && lineCount(voiceCss) < 450 && lineCount(oralHistoryCss) < 600 && lineCount(capsuleCss) < 650 && lineCount(sharePrivacyCss) < 200 && lineCount(mediaEvidenceCss) < 500 && lineCount(exhibitionsCss) < 800 && lineCount(revisitsCss) < 450 && lineCount(revisionsCss) < 350 && lineCount(cluesCss) < 550 && lineCount(collectionHealthCss) < 300 && lineCount(timeCalibrationCss) < 500 && lineCount(archaeology) < 900 && lineCount(archaeologyBackup) < 300]
];
...
function hasVercelSecurityHeaders(config) {
  ...
  return headers["X-Content-Type-Options"] === "nosniff"
    ...
    && headers["Content-Security-Policy"] === "default-src 'self'; connect-src 'self'; img-src 'self' data: blob:; media-src 'self' blob:; style-src 'self'; script-src 'self' 'wasm-unsafe-eval'; worker-src 'self'; manifest-src 'self'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'";
}
