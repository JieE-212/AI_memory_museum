...
const checks = [
  ...
  ["V17 按意思找回留在讲解与回顾且默认折叠", (html.match(/class="nav-button/g) || []).length === 4 && html.includes('<details class="semantic-recall-card" id="semanticRecallDetails">') && !html.includes('<details class="semantic-recall-card" id="semanticRecallDetails" open') && html.indexOf('id="semanticRecallDetails"') > html.indexOf('data-view-panel="reflect"') && !html.includes('data-view="semantic-recall"')],
  ...
  ["V17.1 移动核心旅程沿用四路由并显示小白标签", ["馆藏", "记录", "找回", "我的"].every((label) => html.includes(`aria-label="${label}"`)) && ["collection", "compose", "reflect", "data"].every((view) => html.includes(`data-view="${view}"`)) && html.includes("记得名字、地点或原句") && html.includes("只记得一个画面") && html.includes("想跨多件记忆提问") && html.includes("更多回看方式") && html.includes("项目幕后：完整安全合同")],
