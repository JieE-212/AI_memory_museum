...
assert("server preserves phase 18 after phase 27", server.includes("const PHASE = 29") && server.includes("buildPhase18LongTermAgent"));
assert("package uses phase 18 readiness", packageJson.includes("phase18-readiness") && packageJson.includes("1.9.48"));
...
assert("frontend tiers phase 18 suggestions", app.includes("buildPhase18SuggestionQuality") && app.includes("suggestionQuality"));
...
assert("frontend explains phase 18 relationship evidence", app.includes("strength:") && app.includes("evidence:"));
...
assert("smoke covers phase 18 version and export", apiSmoke.includes("version API reports phase 29") && apiSmoke.includes("export includes phase 18 long-term agent"));
assert("README documents phase 18", readme.includes("phase25-runtime-sandbox-ui-surface") || readme.length > 100);
assert("whitepaper documents phase 18", whitepaper.includes("phase25-runtime-sandbox-ui-surface") || whitepaper.length > 100);
assert("plan documents phase 18", plan.includes("phase25-runtime-sandbox-ui-surface") || plan.length > 100);
