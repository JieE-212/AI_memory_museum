...
async function main() {
  ...
  check(html.includes('rel="manifest" href="/manifest.webmanifest?v=7.1.0"'), "页面声明带版本的 Manifest");
  ...
  const appScript = html.indexOf('/assets/pwa.js?v=7.1.0');
  ...
  check(appScript > html.indexOf('/assets/capsules.js?v=7.1.0') && appScript < html.indexOf('/assets/app.js?v=7.1.0'), "PWA 模块独立且在主应用前载入");
  ...
  check(shellBlock.includes("OFFLINE_URL") && shellBlock.includes('"/pwa.css?v=7.1.0"'), "Service Worker 只预缓存离线边界壳");
...
async function checkWorkerRuntime() {
  ...
  const cachesApi = {
    ...
    keys: async () => ["unrelated-cache", "time-isle-public-shell-v7.0.0", "time-isle-public-shell-v7.1.0"],
  ...
  await installWork;
  deepEqual(addedAssets, ["/offline.html", "/pwa.css?v=7.1.0", "/assets/time-isle-icon.svg"], "Worker 安装只写入三项公开离线壳资源");
  ...
  equal(await dispatchWorkerFetch(listeners, { method: "GET", mode: "cors", url: "https://demo.example/pwa.css?v=7.1.0" }), "cached-pwa-css", "唯一公开壳样式可从缓存读取");
