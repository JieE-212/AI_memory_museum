...
const PRIMARY_ORIGIN = "https://time-isle-demo-284723-8-1456049152.sh.run.tcloudbase.com";
const PRIMARY_URL = `${PRIMARY_ORIGIN}/#collection`;
const TECHNICAL_URL = `${PRIMARY_ORIGIN}/#data-technical`;
const FALLBACK_URL = "https://time-isle-demo-284723-8-1456049152.sh.run.tcloudbase.com/#collection";
let assertions = 0;
...
check("wakeup page exposes one polite atomic status and two explicit domestic routes", (html.match(/role="status"/gu) || []).length === 1 && html.includes('aria-live="polite"') && html.includes('aria-atomic="true"') && html.includes('id="wakeupButton"') && html.includes('id="directLink"') && html.includes('id="fallbackLink"') && html.includes("进入只读安全体验") && html.includes("查看项目技术证据") && html.includes("国内备用入口"));
...
check("wakeup copy discloses resource and privacy boundaries", html.includes("最小实例为 0") && html.includes("最多进行 3 次") && html.includes("不会保存浏览记录") && html.includes("公开只读面试 Demo") && html.includes("设备语义模型也不会自动下载") && html.includes("国内备用入口") && !html.includes("Vercel 备用入口"));
...
check("first attempt uses the fixed cache-busted PNG without requiring CORS", successHarness.images.length === 1 && /^https:\/\/time-isle-demo-284723-8-1456049152\.sh\.run\.tcloudbase\.com\/assets\/time-isle-192\.png\?wake=1000-1$/u.test(successHarness.images[0].src) && successHarness.images[0].referrerPolicy === "no-referrer" && successHarness.images[0].crossOrigin === undefined);
