...
6. 继续工程卫生：保持 `npm.cmd run check` 通过，清理文档编码污染，确认 git 目录扁平化迁移策略。
7. 收口 Phase 29：只有 blocker clearance、redline resolution 和 release approval 明确完成后，才能讨论 Phase 29 exit。
8. 准备 Phase 30 entry：只在人工签核和发布批准完成后建立真实 Phase 30 入口。
9. 运行时启用仍后置：runtime execution、third-party execution、secret access 和 private memory access 必须继续关闭，直到后续明确授权。
