...
8. 继续工程卫生：保持 `npm.cmd run check` 通过，清理文档编码污染，确认 git 目录扁平化迁移策略。
9. 收口 Phase 29：只有 blocker clearance、redline resolution 和 release approval 明确完成后，才能讨论 Phase 29 exit。
10. 准备 Phase 30 entry：只在人工签核、发布批准和独立 Phase 29 exit 完成后建立真实 Phase 30 入口。
11. 运行时启用仍后置：runtime execution、third-party execution、secret access 和 private memory access 必须继续关闭，直到后续明确授权。
