...
V14 公开 Demo 已部署这条 60 秒主线所需的界面与 API，可优先使用 CloudBase 国内主入口演示；桌面公网已实际确认首页、`/api/version`、`/api/health`、`/api/demo/status`、`/api/memories`，Vercel 保留为全球备用。手机 Wi-Fi 与蜂窝网络尚未真机验收，面试前应由用户补测，完成前不要声称移动网络已验证。Vercel 既有深度生产核验还确认策展 sample 为 `synthetic + demo`，调用前后 `runs` 仍为 `0 → 0`；锁馆和结构演练使用虚构 `text/plain` 正文的生产探针均返回 `403 / MUSEUM_LOCK_DEMO_READ_ONLY / bodyBytesRead: 0`，前后 stats、锁状态和 runs 不变。CloudBase 本轮只声明首页与四个只读 API 的实际核验结果。面试前仍按下方清单目视复核页面，并保留本地后备。公开模式不请求麦克风、不打开文件选择，并拒绝策展助手全部非只读请求以及口述史、校准、展览、胶囊、声音或长期回访意愿写入。不要在公开页面上传私人内容，也不要把禁写保护描述成缺失功能。
...
- **线上已经是 V14 吗？** 是。2026-07-20，功能提交 `2dcce402b13f1d43c54c6a196b8e2273c9483eb3` 已进入 GitHub/Gitee 对齐的 `main`；CloudBase 国内入口的部署 `001` 来自提交 `278f925`，桌面公网接口核验为 `14.0.0 / schema 19 / 4` 条播种记忆，Vercel 继续作为全球备用。手机 Wi-Fi 与蜂窝网络真机验收仍待用户完成。Vercel 既有深度核验中，策展 sample 为 `synthetic + demo` 且 runs 为 0；锁馆和结构演练虚构探针均以 `403 / MUSEUM_LOCK_DEMO_READ_ONLY / bodyBytesRead: 0` 拒绝，前后 stats、锁状态和 runs 不变。CloudBase 本轮仅核验首页与四个只读 API；这些结果都不代表公开环境全局零写，也不代表它开放完整写能力。
...
7. CloudBase 的手机 Wi-Fi 与蜂窝网络仍是用户真机待验收项；未补测前只陈述“桌面公网已通过”，不要扩展为“移动端网络已通过”。

V14 已于 2026-07-20 完成代码与 Vercel 发布闭环：功能提交 `2dcce402b13f1d43c54c6a196b8e2273c9483eb3`、GitHub/Gitee `main` 对齐、完整 build/check、262 条真实 HTTP smoke、15/15 项三档 Playwright、同三档真实目视、Vercel 版本与策展 sample/锁馆/结构演练三项零写核验均已完成。CloudBase 部署 `001` 已完成桌面公网首页和四 API 验收，手机 Wi-Fi / 蜂窝网络真机验收仍待用户完成。面试中可以准确表述“V14 已发布、CloudBase 国内桌面公网可访问”，同时必须继续说明线上只是受保护的共享临时 Demo，完整可写能力只在隔离本地 V14 中演示；V10 的数字只保留为历史基线。
