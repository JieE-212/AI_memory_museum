# 时屿面试展示手册：公开 V14 与本地 V17 候选

> 状态（2026-07-21）：公开网址、GitHub 与 Gitee 仍是已发布的 `V14.0.0 / schema 19`；CloudBase 是国内主入口，Vercel 是全球备用。当前本地工作区是 `V17.0.0` 候选版（schema 仍为 19），V15–V17 已完成实现与验收，但尚未提交、推送或部署。V18 只完成 Windows x64 SQLite3MC 可行性 canary，因缺少 Linux / CloudBase 同等证据而总体 **NO-GO**；当前项目没有实现真正的数据库或媒体静态加密。
...
V14 公开 Demo 已部署这条 60 秒主线所需的界面与 API，可优先使用 CloudBase 国内主入口演示；桌面公网已实际确认首页、`/api/version`、`/api/health`、`/api/demo/status`、`/api/memories`，手机 Wi-Fi 与蜂窝网络也已分别确认入口可打开，Vercel 保留为全球备用。移动端结果只证明两类网络下入口可达；缩零后的首次冷启动与临时写入消失仍待单独观察。Vercel 既有深度生产核验还确认策展 sample 为 `synthetic + demo`，调用前后 `runs` 仍为 `0 → 0`；锁馆和结构演练使用虚构 `text/plain` 正文的生产探针均返回 `403 / MUSEUM_LOCK_DEMO_READ_ONLY / bodyBytesRead: 0`，前后 stats、锁状态和 runs 不变。CloudBase 本轮只声明桌面首页与四个只读 API 的实际核验，以及手机两类网络的入口可达结果。面试前仍按下方清单目视复核页面，并保留本地后备。公开模式不请求麦克风、不打开文件选择，并拒绝策展助手全部非只读请求以及口述史、校准、展览、胶囊、声音或长期回访意愿写入。不要在公开页面上传私人内容，也不要把禁写保护描述成缺失功能。
...
这条路线只在隔离本地 V17 候选版使用虚构数据，不能指着公开 V14 网址讲成已上线：
...
本地候选已通过 build/check、272 条真实 HTTP smoke、21/21 项 Playwright 三档门禁和桌面、390×844、320×700 真实浏览器验收；真实 q8 ONNX/WASM 推理成功，且专项检查未发现第三方请求或浏览器持久存储新增。这些数字不属于线上 V14 的生产结果。
...
11. 若展示本地候选，再依次演示 V15 一次性真实恢复、V16 多视角只读对照和 V17 设备内“按意思找回”；三项均默认折叠并保留人工边界。
...
本地 V17 候选继续补上三个闭环：V15 把备份真正恢复到一次性 SQLite 与媒体副本，独立体检后销毁；V16 只读并列当前记录、编辑年轮、已确认回信和人工来源关系，不判断谁更真实；V17 由用户主动加载同源中文 embedding，在本页 Worker 内存按文字含义找回。它们尚未发布。V18 则因缺少 Linux / CloudBase 证据主动停在 NO-GO，不能用现有写门禁冒充真正加密。
...
- **线上已经是 V14 吗？** 是。2026-07-20，功能提交 `2dcce402b13f1d43c54c6a196b8e2273c9483eb3` 已进入 GitHub/Gitee 对齐的 `main`；CloudBase 国内入口的部署 `001` 来自提交 `278f925`，桌面公网接口核验为 `14.0.0 / schema 19 / 4` 条播种记忆，手机 Wi-Fi 与蜂窝网络入口可达也已由用户真机确认，Vercel 继续作为全球备用。缩零后的冷启动仍待单独观察。Vercel 既有深度核验中，策展 sample 为 `synthetic + demo` 且 runs 为 0；锁馆和结构演练虚构探针均以 `403 / MUSEUM_LOCK_DEMO_READ_ONLY / bodyBytesRead: 0` 拒绝，前后 stats、锁状态和 runs 不变。CloudBase 本轮仅核验首页与四个只读 API，以及手机两类网络的入口可达；这些结果都不代表公开环境全局零写，也不代表它开放完整写能力。
- **V15–V17 已经上线了吗？** 没有。它们已经在本地 V17 候选版完成实现与验收，但尚未提交、推送或部署；公开链接仍是 V14。
...
1. 优先打开 CloudBase 国内入口：首页应正常；`/api/version` 应返回 `version: 14.0.0`，`/api/health` 应返回 `schemaVersion: 19`、`mode: interview-demo`、`storage: ephemeral-sqlite` 和 `aiMode: mock-fallback`，`/api/memories` 应返回 4 条播种记忆；再核对 `/api/demo/status`。若展示策展 sample 或锁馆、结构演练生产探针，只引用 Vercel 的既有深度核验记录：sample 为 `synthetic: true / demo: true / runs 0 → 0`，两项虚构 `text/plain` 探针均为 `403 / MUSEUM_LOCK_DEMO_READ_ONLY / bodyBytesRead: 0`，前后 stats、锁状态和 runs 不变。不要把这些 Vercel 结果转述为 CloudBase 本轮已执行。
2. 开场明确“线上是已发布的 V14 共享临时 Demo；V15–V17 只是本地 V17 候选，尚未发布”；不同版本的 smoke、Playwright 与生产核验数字不要混用。
3. 用隔离测试数据库启动本地 V17，只保留完全虚构的照片、声音、共忆问答、来源关系、展览、时间判断和至少两版展品。
...
7. CloudBase 的手机 Wi-Fi 与蜂窝网络入口可达已由用户真机确认；只陈述“两类移动网络均可打开入口”，不要扩展为所有移动端交互或缩零恢复场景都已验收。缩零冷启动仍待单独观察。

V14 已于 2026-07-20 完成双远端与生产发布闭环，CloudBase 是国内简历主入口，Vercel 是全球备用。V17 候选版已在本地完成 V15–V17 与整库验收，但尚未提交、推送或发布；V18 停在总体 NO-GO 门禁。面试时最重要的版本口径是：**线上 V14 已发布，本地 V17 候选已验收，V18 真正加密未实现。**
