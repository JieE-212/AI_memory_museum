# 时屿面试展示手册：公开 V17 与本地完整能力

> 状态（2026-07-22）：公开网址、GitHub 与 Gitee 已发布 `V17.0.0 / schema 19`；功能提交为 `413f78640baad7eae6324ef14bc291f05325fbf6`，热修复为 `38d3450b9f26efdc59df4860317cdf9513e77e65`。CloudBase 部署 `002` 与 Vercel 均完成桌面/接口生产核验；独立静态应用 `time-isle-wakeup-002` 已提供国内唤醒入口，并由用户确认手机 Wi-Fi / 蜂窝入口链路均可进入 V17。Vercel 是全球备用。V18 只完成 Windows x64 SQLite3MC 可行性 canary，因缺少 Linux / CloudBase 同等证据而总体 **NO-GO**；当前项目没有实现真正的数据库或媒体静态加密。

## 一句话定位

时屿不是“可以放图片的备忘录”，而是一套本地优先的私人记忆策展工具：它让原文、多模态证据和他人见证保持独立，用可解释对照与设备内语义帮助找回，并把策展、关系、分享、锁馆和恢复判断都留给用户明确决定。
...
- 全球备用入口（Vercel）：`https://ai-memory-museum-demo.vercel.app`
- 本地后备：`http://127.0.0.1:3000/#reflect`，只使用虚构测试数据

| 时间 | 操作 | 讲什么 |
| --- | --- | --- |
| 0–8 秒 | 点击“唤醒并进入”，展示线上首页和四项导航 | 静态入口先解释缩零与免费边界；主应用始终只有四个任务，公开环境禁止私人媒体写入。 |
...
V17 公开 Demo 已部署这条 60 秒主线所需的界面与 API，可优先使用 CloudBase 国内静态唤醒入口演示。CloudBase 部署 `002` 已实际确认首页、`17.0.0 / schema 19`、4 件播种记忆、设备语义、多视角、隔离恢复禁写、Worker 与模型资产；页面显示 `v17.0.0` 且控制台零错误。空闲后的首次请求曾短暂返回 503，约 5 秒后恢复；静态入口因此采用用户点击、3 次有限图片探针和自动跳转，24/24 项三档门禁已通过。Vercel 完成同版本、语义、多视角、隔离恢复和静态资产核验。用户已在手机 Wi-Fi 与蜂窝网络下确认静态入口可达并能进入 V17；该结果只代表入口链路验收，不扩大为全部移动端功能逐项通过。面试前仍按下方清单目视复核页面，并保留本地后备。公开模式不请求麦克风、不打开文件选择，并拒绝 V15 完整备份上传、策展助手全部非只读请求以及口述史、校准、展览、胶囊、声音或长期回访意愿写入。
...
## V12–V14 线上边界与本地完整能力 90 秒路线（延续至 V17）

线上只展示设备内镜片以及共忆、锁馆和结构演练的只读 / 禁写边界；以下完整交互必须切到隔离本地环境并只使用虚构数据。开场明确说“线上是受保护的共享临时 Demo，这些模块保持零写；本地才开放完整可写能力”：
...
V16 多视角与 V17“按意思找回”可在公开 V17 的播种数据上展示；V15 真恢复必须切到隔离本地环境并使用虚构完整备份，不能因为代码已发布就声称公开 Demo 接收私人归档：

1. 在“数据与项目”展开恢复演练，选择虚构完整 `.time-isle`，展示“验真 → 恢复一次性副本 → 独立体检 → 销毁副本”。强调当前馆藏没有被替换，结果只证明当前机器和当前版本可恢复，不是异机灾备。
2. 打开一件展品的“多视角记忆对照”，并列当前记录、编辑年轮、已确认亲友回信与来源关系。指出只有人工确认关系才进入对照，身份未核验、未绑定或待复核状态不会被系统裁决。
3. 回到“讲解与回顾”展开“按意思找回”。先说明用户主动点击后才加载约 47 MB 同源中文模型，再用自然描述搜索；查询、短文本、索引和 512 维向量只留在本页 Worker 内存。最后点击释放，并提醒最接近也可能不相关。
4. 收尾展示四项导航和移动固定底栏：三个新能力都放回已有任务流并默认折叠，没有把页面扩张成工具面板。

V17 发布版已通过 build/check、272 条真实 HTTP smoke、21/21 项 Playwright 三档门禁和桌面、390×844、320×700 真实浏览器验收；真实 q8 ONNX/WASM 推理成功，且专项检查未发现第三方请求或浏览器持久存储新增。生产核验只覆盖上文实际执行的 V17 UI/API/静态资产项目；手机 Wi-Fi / 蜂窝结果只覆盖入口链路，两者都不能扩大成公开写能力或全部移动端功能逐项结论。
...
11. 依次演示 V15 一次性真实恢复、V16 多视角只读对照和 V17 设备内“按意思找回”；三项均默认折叠并保留人工边界，其中 V15 完整上传只在隔离本地环境进行。
...
V12–V14 再把“他人见证、换角度回看和暂停写入”做成三个克制模块。共忆信笺在浏览器端加密请求与回信，但身份始终未验证、文件未签名，入馆后也只是独立来源；设备内镜片只用确定性规则重排保存态，零模型、零工具、零保存；锁馆只在正文前阻止新的应用写入，不加密 SQLite 或媒体。结构演练也只验 manifest、哈希和引用，没有执行恢复，更不证明灾难恢复能力。这些能力从 V14 延续到 V17；线上公开 Demo 只开放合成/播种数据上的只读预览和禁写边界，隔离本地环境才开放使用虚构数据演示的完整可写流程。

V17 发布版补上三个闭环：V15 把备份真正恢复到一次性 SQLite 与媒体副本，独立体检后销毁；V16 只读并列当前记录、编辑年轮、已确认回信和人工来源关系，不判断谁更真实；V17 由用户主动加载同源中文 embedding，在本页 Worker 内存按文字含义找回。公开 Demo 仍在正文前拒绝 V15 完整上传。V18 则因缺少 Linux / CloudBase 证据主动停在 NO-GO，不能用现有写门禁冒充真正加密。
...
数据默认保存在本地 SQLite 和媒体目录。V17 schema 19 的完整 `.time-isle` 延续收件箱、来源护照和共忆回信，但锁状态、salt、digest 与 verifier 始终不进入普通归档。公开 Demo 已是 V17，但仍是共享、临时、受保护的面试环境；核心纯文本仅可在硬上限内临时体验，高风险写操作和 V15 完整上传保持只读或禁用。项目没有账号、云同步、万能 OCR、自动语音理解、磁盘静态加密或自治发布，这些是明确边界。
...
flowchart TB
  ...
  subgraph B["浏览器私密区"]
    UI --> MOD["影像 · 声音 · 共忆 · 对照 · 设备语义 · 口述史 · 受限策展 · 恢复"]
    MOD --> LOCAL["Canvas · TextDetector · Web Crypto · Worker / ONNX WASM"]
  end

  MOD -->|"fetch / 二进制流"| SEC["请求安全边界<br/>Host · Origin · Fetch Metadata"]
  SEC --> HTTP["同一 Node request handler<br/>server.js / api/index.js"]
  HTTP --> DOMAIN["领域层<br/>记忆检索 · 多视角对照 · 口述史 · 媒体声音 · 只读策展 · 迁移归档"]
  ...
  HTTP --> LOCK["写门禁与恢复演练<br/>结构验真 · 一次性恢复副本 · 销毁回执"]
  GATE --> LOCAL
...
| 一次性真实隔离恢复 | 完整备份真正写入独立 SQLite、图片和声音副本，复核后销毁；当前馆藏不参与，清理失败时闭锁后续演练。 | [`lib/isolated-recovery-drill.js`](../lib/isolated-recovery-drill.js)、[`lib/isolated-recovery-runtime.js`](../lib/isolated-recovery-runtime.js)、[`scripts/isolated-recovery-check.js`](../scripts/isolated-recovery-check.js) |
| 人工关系驱动的多视角对照 | 当前记录、年轮、已确认回信和来源护照只读并列；未绑定、撤回、身份未核验或来源变化不会被包装成结论。 | [`lib/multi-perspective-service.js`](../lib/multi-perspective-service.js)、[`public/assets/multi-perspective.js`](../public/assets/multi-perspective.js)、[`scripts/multi-perspective-check.js`](../scripts/multi-perspective-check.js) |
| 设备内中文语义找回 | 同源 q8 模型在 Worker 内生成临时 512 维 embedding；用户主动加载，可停止和释放，不发第三方、不持久化向量。 | [`lib/semantic-recall-service.js`](../lib/semantic-recall-service.js)、[`public/assets/semantic-recall-worker.js`](../public/assets/semantic-recall-worker.js)、[`scripts/semantic-recall-worker-check.js`](../scripts/semantic-recall-worker-check.js) |
...
- **展品库搜索和“按意思找回”有什么区别？** 展品库是 FTS5 trigram、短词参数化 LIKE、字段加权和实体线索，因此叫“字段与线索检索”；“按意思找回”才会在用户主动启用后，用同源自托管的中文 embedding 在浏览器 Worker 内按文本相似度排序。首版不使用向量数据库，也不把相似度解释为事实或概率。
...
- **结构验真和 V15 一次性恢复有什么区别？** 结构验真只读检查归档合同；V15 会把备份真正写入独立 SQLite、图片和声音副本，复核后销毁。它仍只证明当前机器、当前版本可恢复，不证明异机灾备。
- **多视角对照会判断谁记得更准吗？** 不会。它只呈现已有记录和人工确认的支持、补充或不同记录关系；亲友身份仍未核验，来源变化只进入待复核。
- **“按意思找回”会上传原文吗？** 不会。模型资源同源自托管，查询、短文本、索引和向量只在本页 Worker 内存处理；结果只是相似度排序，最接近也可能不相关。
...
- **V10 现在是什么状态？** V10.0.0 / schema 14 是 2026-07-19 的历史发布基线；当时的 249 条 smoke、具体专项数字、双远端和生产核验只用于说明版本演进，不应当作 V17 的当前测试或线上结果。
- **线上已经是 V17 吗？** 是。功能提交 `413f78640baad7eae6324ef14bc291f05325fbf6` 与热修复 `38d3450b9f26efdc59df4860317cdf9513e77e65` 已进入 GitHub/Gitee `main`；CloudBase 部署 `002` 正常并承接 100% 流量，Vercel 与 CloudBase 均返回 `17.0.0 / schema 19`。CloudBase 已核验 UI、4 件播种记忆、设备语义、多视角、隔离恢复禁写和模型资产；用户也已确认手机 Wi-Fi / 蜂窝静态入口链路可进入 V17。V14 的部署 `001`、手机结果与深度探针只保留为历史证据。
- **V15–V17 已经上线了吗？** 已随 V17 发布，但公开 Demo 仍受保护：V16/V17 只在播种数据上提供安全只读体验，V15 完整备份上传与真实隔离恢复仍只在隔离本地环境开放。
- **V18 已经实现真正加密了吗？** 没有。Windows SQLite3MC canary 通过，但 Linux / CloudBase 门禁缺失，因此总体 NO-GO；没有接入正式数据层，也不能把 SQLite3MC 称为 Zetetic 官方 SQLCipher。

## 面试前一分钟检查

1. 优先打开 CloudBase 静态唤醒入口：页面应立即出现且不会自动请求云托管唤醒探针；点击后应进入 V17。再确认 `/api/version` 返回 `version: 17.0.0`、`/api/health` 返回 `schemaVersion: 19 / interview-demo / ephemeral-sqlite / mock-fallback`，并展开“按意思找回”核对约 47 MB 边界。
2. 开场明确“线上是已发布的 V17 共享临时 Demo；V15 完整恢复只在隔离本地环境开放，V18 尚未实现真正加密”；不同版本的 smoke、Playwright 与生产核验数字不要混用。
3. 若演示完整可写能力，用隔离测试数据库启动本地 V17，只保留完全虚构的照片、声音、共忆问答、来源关系、展览、时间判断和至少两版展品。
4. 预先准备并验过一份虚构 schema 19 完整 `.time-isle`；分别确认结构演练只显示结构验真，以及 V15 只恢复一次性副本并最终显示销毁完成。
5. 关闭系统通知；演示录音时提前确认麦克风权限，公开 Demo 不请求麦克风。
6. 网络或浏览器能力失败时直接走本地后备路线，不临时改配置或上传私人文件。
7. V17 的 CloudBase 静态入口已由用户在手机 Wi-Fi 与蜂窝网络下确认可达并能进入主应用；只陈述“入口链路已通过”，不要扩写为录音、记录、馆藏、回望等全部移动端功能逐项验收。缩零冷启动窗口已经观察到，临时内容消失仍待单独验证。

V17 已于 2026-07-22 完成 Vercel 与 CloudBase 主应用发布，CloudBase 静态唤醒页已通过桌面与手机 Wi-Fi / 蜂窝入口链路验收，现作为国内简历主入口；Vercel 是全球备用。公开 Demo 继续禁用 V15 完整上传，V18 停在总体 NO-GO 门禁。面试时最重要的版本口径是：**线上 V17 已发布，完整恢复只在本地虚构数据演示，V18 真正加密未实现。**
