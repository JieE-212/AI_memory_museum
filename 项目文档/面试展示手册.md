...
V17 公开 Demo 已部署这条 60 秒主线所需的界面与 API，可优先使用 CloudBase 国内静态唤醒入口演示。CloudBase 部署 `002` 已实际确认首页、`17.0.0 / schema 19`、4 件播种记忆、设备语义、多视角、隔离恢复禁写、Worker 与模型资产；页面显示 `v17.0.0` 且控制台零错误。空闲后的首次请求曾短暂返回 503，约 5 秒后恢复；静态入口因此使用用户点击、3 次有限图片探针和自动跳转，24/24 项三档门禁已通过。Vercel 完成同版本、语义、多视角、隔离恢复和静态资产核验。2026-07-20 的手机 Wi-Fi / 蜂窝入口可达只属于 V14 历史结果，V17 静态入口与主应用真机尚未复核。面试前仍按下方清单目视复核页面，并保留本地后备。公开模式不请求麦克风、不打开文件选择，并拒绝 V15 完整备份上传、策展助手全部非只读请求以及口述史、校准、展览、胶囊、声音或长期回访意愿写入。
...
V17 发布版已通过 build/check、272 条真实 HTTP smoke、21/21 项 Playwright 三档门禁和桌面、390×844、320×700 真实浏览器验收；真实 q8 ONNX/WASM 推理成功，且专项检查未发现第三方请求或浏览器持久存储新增。生产核验只覆盖上文实际执行的 V17 UI/API/静态资产项目，不把本地完整恢复门禁或三档视口扩大成公开写能力与手机真机结论。
...
- **线上已经是 V17 吗？** 是。功能提交 `413f78640baad7eae6324ef14bc291f05325fbf6` 与热修复 `38d3450b9f26efdc59df4860317cdf9513e77e65` 已进入 GitHub/Gitee `main`；CloudBase 部署 `002` 正常并承接 100% 流量，Vercel 与 CloudBase 均返回 `17.0.0 / schema 19`。CloudBase 已核验 UI、4 件播种记忆、设备语义、多视角、隔离恢复禁写和模型资产。V17 手机真机尚未复核；V14 的部署 `001`、手机结果与深度探针只保留为历史证据。
...
7. V14 的 CloudBase 手机 Wi-Fi 与蜂窝网络入口可达曾由用户真机确认，但不能继承为 V17 验收；V17 重新真机复核前，只陈述桌面 UI/API、静态唤醒链路和三档真实浏览器视口已通过。缩零冷启动窗口已经观察到，临时内容消失仍待单独验证。

V17 已于 2026-07-22 完成 Vercel 与 CloudBase 主应用发布，CloudBase 静态唤醒页是国内简历入口候选，Vercel 是全球备用；公开 Demo 继续禁用 V15 完整上传，V17 手机真机尚未复核。V18 停在总体 NO-GO 门禁。面试时最重要的版本口径是：**线上 V17 已发布，完整恢复只在本地虚构数据演示，V18 真正加密未实现。**
