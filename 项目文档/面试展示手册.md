...
> 状态（2026-07-20）：`V14.0.0 / schema 19` 已发布，功能提交为 `2dcce402b13f1d43c54c6a196b8e2273c9483eb3`，GitHub 与 Gitee 的 `main` 已对齐。完整 build/check、262 条真实 HTTP smoke、15/15 项三档 Playwright 与同三档真实目视均已通过；CloudBase 服务 `time-isle-demo` 的部署 `001` 来自提交 `278f925`，桌面公网首页和四个 API 已核验为 `14.0.0 / schema 19 / 4` 条播种记忆，手机 Wi-Fi 与蜂窝网络入口可达也已由用户真机确认，Vercel 继续作为全球备用入口。CloudBase 保持 `0.5` 核 / `1 GiB`、`0–1` 个实例、`/tmp` 临时存储与关闭按量付费；缩零后的首次冷启动与临时写入消失仍待单独观察。面试时应明确区分“线上 V14 受保护的共享临时 Demo”和“本地 V14 完整可写能力”：前者只允许硬上限内的核心纯文本体验，V11–V14 新模块与高风险写操作保持只读或禁用。V10 的 249 条 smoke 与具体专项数字只能作为历史基线。
...
V14 已于 2026-07-20 完成代码与 Vercel 发布闭环：功能提交 `2dcce402b13f1d43c54c6a196b8e2273c9483eb3`、GitHub/Gitee `main` 对齐、完整 build/check、262 条真实 HTTP smoke、15/15 项三档 Playwright、同三档真实目视、Vercel 版本与策展 sample/锁馆/结构演练三项零写核验均已完成。CloudBase 部署 `001` 已完成桌面公网首页和四 API 验收，手机 Wi-Fi / 蜂窝网络入口可达也已由用户真机确认。面试中可以准确表述“V14 已发布、CloudBase 国内主入口可由桌面及两类手机网络打开”，并把 CloudBase 作为最终简历主入口、Vercel 作为全球备用；同时必须继续说明缩零冷启动尚待观察，线上也只是受保护的共享临时 Demo，完整可写能力只在隔离本地 V14 中演示。V10 的数字只保留为历史基线。
