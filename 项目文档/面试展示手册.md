...
> 状态（2026-07-19）：`V10.0.0 / schema 14` 代码发布提交 `7107ede` 已推送至 GitHub 与 Gitee，并部署到生产 Demo。线上 `/api/health` 与 `/api/version` 已核验为 `V10.0.0 / schema 14 / interview-demo / ephemeral-sqlite`；`/api/curator-agent/sample` 返回 `synthetic: true`、`demo: true`，调用前后持久化 `runs` 计数保持 `0 → 0`。`npm.cmd run build`、`npm.cmd run check` 全绿，真实 HTTP smoke 249 条；策展 core 131、归档 98、UI 51、数据库健康 42、馆藏体检 83、Demo 安全 19、前端 109、JSON 导入 160、归档验真 42、媒体备份 312/恢复 189 条专项均已通过。1265×720、390×844、320×700 的浏览器 DOM 与真实交互验收也已通过，并非仅凭截图判断。
...
V10 公开 Demo 已部署这条 60 秒主线所需的界面与 API，可优先按全线上路线演示；发布核验已实际确认版本接口与策展 sample 为 `synthetic + demo`，调用前后 `runs` 仍为 `0 → 0`。面试前仍按下方清单目视复核页面，并保留本地后备。公开模式不请求麦克风、不打开文件选择，并拒绝策展助手全部非只读请求以及口述史、校准、展览、胶囊、声音或长期回访意愿写入。不要在公开页面上传私人内容，也不要把禁写保护描述成缺失功能。
...
数据默认保存在本地 SQLite 和媒体目录。馆藏体检只读核对数据库、图片、声音、口述史、时间判断与策展四表；schema 14 的 `.time-isle` 还携带 `curator-agent/state.json`，但恢复后强制成为 `historical + needsReview + allowDecisions: false`，不能重放旧批准。非空口述 JSON 仍只返回 `requiresTimeIsle`，避免恢复半套回答。公开 Demo 已是 V10；sample 为内存合成且全部非只读策展请求 403。项目没有账号、云同步、万能 OCR、自动语音理解或自治发布，这些是明确边界。
...
- **线上已经是 V10 吗？** 是。2026-07-19 已完成 GitHub、Gitee 双远端推送和生产 Demo 部署；线上接口已核验 `V10.0.0 / schema 14 / interview-demo / ephemeral-sqlite`，策展 sample 为 `synthetic + demo` 且调用前后 `runs` 保持 `0 → 0`。这证明版本与公开 Demo 边界已经生效，不代表公开环境允许保存私人内容或执行受保护写操作。

## 面试前一分钟检查

1. 打开线上 `/api/health` 与 `/api/version`，确认 `version: 10.0.0`、`schemaVersion: 14`、`mode: interview-demo` 和 `ephemeral-sqlite`；打开策展 sample，确认 `synthetic: true`、`demo: true`，且调用前后 `runs` 为 `0 → 0`。
2. 准备门禁摘要：HTTP smoke 249、前端 109、策展 core 131、策展归档 98、策展 UI 51、数据库健康 42、馆藏体检 83、Demo 安全 19、JSON 导入 160、归档验真 42、媒体备份 312/恢复 189、PWA 79。
3. 用隔离测试数据库启动本地服务，只保留完全虚构的照片、声音、至少 1 场已确认展览、1 份策展提案/决定链、1 项时间校准、1 个口述问题/回答链、胶囊、回访意愿和至少两版展品。
4. 预先准备并验过一份虚构 schema 14 `.time-isle`，打开线上展品库与线上“讲解与回顾”；只有展示可写流程、馆藏体检或恢复时才打开本地“数据与项目”和离线 HTML 标签。
5. 关闭系统通知；演示录音时提前确认麦克风权限，公开 Demo 不请求麦克风。
6. 网络或浏览器能力失败时直接走本地后备路线，不临时改配置或上传私人文件。

V10 已完成 `npm.cmd run build`、`npm.cmd run check`、249 条真实 HTTP smoke，以及 1265×720、390×844、320×700 的浏览器 DOM+真实交互验收。三档实测覆盖四导航/移动底栏、无横向溢出、全屏工作台、逐项保存/关系/发布、隐私编辑台默认零素材、最近记录只读打开/离线重放和删除二次确认。双远端推送、生产部署与线上接口复核也已完成，可以在简历和面试中准确表述 V8–V10 已发布；同时必须保留“公开 Demo 临时、禁私人媒体写入、策展 sample 零持久化”的边界。
