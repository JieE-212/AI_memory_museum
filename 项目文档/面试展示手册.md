...
V17 公开 Demo 已部署这条 60 秒主线所需的界面与 API，可优先使用 CloudBase 国内主入口演示。CloudBase 部署 `002` 已实际确认首页、`17.0.0 / schema 19`、4 件播种记忆、设备语义、多视角、隔离恢复禁写、Worker 与模型资产；页面显示 `v17.0.0` 且控制台零错误。Vercel 完成同版本、语义、多视角、隔离恢复和静态资产核验。2026-07-20 的手机 Wi-Fi / 蜂窝入口可达只属于 V14 历史结果，V17 真机尚未复核；缩零冷启动也仍待观察。V14 策展 sample、锁馆和结构演练深度探针继续作为历史证据，不冒充 V17 已重跑。面试前仍按下方清单目视复核页面，并保留本地后备。公开模式不请求麦克风、不打开文件选择，并拒绝 V15 完整备份上传、策展助手全部非只读请求以及口述史、校准、展览、胶囊、声音或长期回访意愿写入。不要在公开页面上传私人内容，也不要把禁写保护描述成缺失功能。
...
1. 优先打开 CloudBase 国内入口：首页应正常；`/api/version` 应返回 `version: 17.0.0`，`/api/health` 应返回 `schemaVersion: 19`、`mode: interview-demo`、`storage: ephemeral-sqlite` 和 `aiMode: mock-fallback`，`/api/memories` 应返回 4 条播种记忆；再展开“按意思找回”并核对约 47 MB 边界。V14 策展 sample、锁馆与结构演练的深度探针只能标成历史结果；V17 本轮可引用设备语义、多视角、隔离恢复禁写与静态资产探针。
...
7. V14 的 CloudBase 手机 Wi-Fi 与蜂窝网络入口可达曾由用户真机确认，但不能继承为 V17 验收；V17 重新真机复核前，只陈述桌面 UI/API 和三档真实浏览器视口已通过。缩零冷启动仍待单独观察。

V17 已于 2026-07-22 完成双远端与 Vercel、CloudBase 发布闭环，CloudBase 是国内简历主入口，Vercel 是全球备用；公开 Demo 继续禁用 V15 完整上传，V17 手机真机尚未复核。V18 停在总体 NO-GO 门禁。面试时最重要的版本口径是：**线上 V17 已发布，完整恢复只在本地虚构数据演示，V18 真正加密未实现。**
