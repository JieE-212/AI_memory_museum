...
Phase 30 entry decision packet: active. 项目已经建立 `2.0.24 / phase30-entry-decision-packet`，用于定义 Phase 30 entry owner 的独立进入记录格式；它只是 phase30-entry-decision-packet-only，not Phase 30 entry，也不能自动触发 runtime go/no-go。

Phase 30 runtime go/no-go decision packet: active. 项目已经建立 `2.0.25 / phase30-runtime-go-no-go-decision-packet`，用于定义 runtime owner 的独立 Go/No-Go 记录格式；它只是 runtime-go-no-go-decision-packet-only，not runtime execution approval，也不能自动触发 third-party execution。

Phase 30 third-party execution approval boundary: active. 项目已经建立 `2.0.26 / phase30-third-party-execution-approval-boundary`，用于定义单插件第三方执行审批边界；它只是 third-party-execution-approval-boundary-only，not third-party execution approval，也不能执行真实插件代码。
...
Phase 30 candidate long-run audit: active。当前已新增 `后续任务长跑执行记录.md` 与 `阶段30候选交付深度审计记录.md`，用于记录脚本矩阵、文档镜像、红线语义、API 状态和保留风险；它们不代表 reviewer 输出、live submission、发布批准或阶段入口。
