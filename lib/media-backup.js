...
const { createArchive, createArchiveFile, extractArchive } = require("./time-isle-archive");
const { inspectImage } = require("./media-format");
const { redactMemory, REDACTED_COLLECTION_PRIVACY } = require("./collection-export");
const { redactArchaeologyBackup, validateArchaeologyBackup } = require("./archaeology-backup");
const {
...
const { REVISION_REDACTED_NOTE } = require("./revision-backup");
const {
  REVISIT_INTENT_REDACTED_NOTE,
  validateRevisitIntentBackupPayload
} = require("./revisit-intent-database");
const { TIME_CALIBRATION_REDACTED_NOTE } = require("./time-calibration-database");
const {
...
const ARCHIVE_PATHS = Object.freeze({
  ...
  revisions: "revisions/state.json",
  revisitIntents: "revisits/intents.json",
  timeCalibrations: "timeline/calibrations.json",
  oralHistories: ORAL_HISTORY_SECTION_PATH,
  curatorAgent: CURATOR_AGENT_SECTION_PATH,
  memoryInbox: MEMORY_INBOX_SECTION_PATH,
  provenance: "provenance/state.json",
  coMemoryResponses: CO_MEMORY_RESPONSE_SECTION_PATH,
...
const FEATURE_ARCHIVE_SECTIONS = Object.freeze([
  ...
  Object.freeze({
    name: "revisions",
    collectionKey: "revisions",
    path: ARCHIVE_PATHS.revisions,
    sinceSchemaVersion: 10,
    version: 1,
    recordsKey: "revisions",
    summaryCountKey: "revisionCount",
    summaryCountKeys: Object.freeze(["revisionCount", "memoryCount"]),
    redactedNote: REVISION_REDACTED_NOTE,
    maximumRecords: 10000
  }),
  Object.freeze({
    name: "revisit-intents",
    collectionKey: "revisitIntents",
    path: ARCHIVE_PATHS.revisitIntents,
    sinceSchemaVersion: 11,
    version: 1,
    recordsKey: "intents",
    summaryCountKey: "intentCount",
    summaryCountKeys: Object.freeze(["intentCount"]),
    redactedNote: REVISIT_INTENT_REDACTED_NOTE,
    maximumRecords: 500
  }),
  Object.freeze({
    name: "time-calibrations",
    collectionKey: "timeCalibrations",
    path: ARCHIVE_PATHS.timeCalibrations,
    sinceSchemaVersion: 12,
    version: 1,
    recordsKey: "calibrations",
    summaryCountKey: "calibrationCount",
    summaryCountKeys: Object.freeze(["calibrationCount", "uncertainCount", "alternativesCount"]),
    redactedNote: TIME_CALIBRATION_REDACTED_NOTE,
    maximumRecords: 500
  }),
  Object.freeze({
    name: ORAL_HISTORY_SECTION_NAME,
    collectionKey: "oralHistories",
    path: ARCHIVE_PATHS.oralHistories,
    sinceSchemaVersion: ORAL_HISTORY_SCHEMA_VERSION,
    version: ORAL_HISTORY_SECTION_VERSION,
    recordsKey: "questions",
    summaryCountKey: "questionCount",
    summaryCountKeys: Object.freeze(["questionCount", "answerCount", "confirmedAnswerCount"]),
    redactedNote: ORAL_HISTORY_REDACTED_NOTE,
    maximumRecords: MAX_ORAL_HISTORY_QUESTIONS,
    customValidator: "oral-history"
  }),
  Object.freeze({
    name: CURATOR_AGENT_SECTION_NAME,
    collectionKey: "curatorAgent",
    path: ARCHIVE_PATHS.curatorAgent,
    sinceSchemaVersion: CURATOR_AGENT_SCHEMA_VERSION,
    version: CURATOR_AGENT_SECTION_VERSION,
    recordsKey: "runs",
    summaryCountKey: "runCount",
    summaryCountKeys: Object.freeze([
      "runCount", "completedRunCount", "cancelledRunCount", "proposalCount",
      "decisionCount", "approvedCount", "rejectedCount"
    ]),
    redactedNote: CURATOR_AGENT_REDACTED_NOTE,
    maximumRecords: CURATOR_AGENT_LIMITS.runs,
    customValidator: "curator-agent"
  }),
  Object.freeze({
    name: MEMORY_INBOX_SECTION_NAME,
    collectionKey: "memoryInbox",
    path: ARCHIVE_PATHS.memoryInbox,
    sinceSchemaVersion: 15,
    version: MEMORY_INBOX_SECTION_VERSION,
    recordsKey: "items",
    summaryCountKey: "itemCount",
    summaryCountKeys: Object.freeze([
      "sourceCount", "itemCount", "pendingCount", "dismissedCount", "acceptedCount", "orphanedCount"
    ]),
    redactedNote: MEMORY_INBOX_REDACTED_NOTE,
    maximumRecords: MAX_MEMORY_INBOX_ITEMS,
    customValidator: "memory-inbox"
  }),
  Object.freeze({
    name: "provenance",
    collectionKey: "provenance",
    path: ARCHIVE_PATHS.provenance,
    sinceSchemaVersion: 16,
    version: 1,
    recordsKey: "claims",
    summaryCountKey: "claimCount",
    summaryCountKeys: Object.freeze([
      "claimCount", "draftCount", "confirmedCount", "needsReviewCount", "withdrawnCount", "sourceCount"
    ]),
    redactedNote: PROVENANCE_REDACTED_NOTE,
    maximumRecords: MAX_BACKUP_CLAIMS,
    customValidator: "provenance"
  }),
  Object.freeze({
    name: CO_MEMORY_RESPONSE_SECTION_NAME,
    collectionKey: "coMemoryResponses",
    path: ARCHIVE_PATHS.coMemoryResponses,
    sinceSchemaVersion: 17,
    version: CO_MEMORY_RESPONSE_SECTION_VERSION,
    recordsKey: "responses",
    summaryCountKey: "responseCount",
    summaryCountKeys: Object.freeze([
      "responseCount", "unverifiedIdentityCount", "encryptedTransportCount", "unsignedCount"
    ]),
    redactedNote: CO_MEMORY_RESPONSE_REDACTED_NOTE,
    maximumRecords: CO_MEMORY_RESPONSE_LIMITS.responses,
    customValidator: "co-memory-response"
  })
]);
...
const RESERVED_ARCHIVE_PREFIXES = Object.freeze(["media/", "exhibitions/", "revisits/", "entities/", "capsules/", "revisions/", "timeline/", "provenance/", CO_MEMORY_RESPONSE_ARCHIVE_PREFIX, MEMORY_INBOX_ARCHIVE_PREFIX, ORAL_HISTORY_ARCHIVE_PREFIX, CURATOR_AGENT_ARCHIVE_PREFIX, VOICE_ARCHIVE_PREFIX]);
const REDACTED_COLLECTION_KEYS = new Set([
  "archaeology", "capsules", "count", "entities", "exhibitions", "exportedAt", "memories",
  "mode", "privacy", "product", "productEnglish", "revisitIntents", "revisions", "revisits", "schemaVersion",
  "coMemoryResponses", "curatorAgent", "memoryInbox", "oralHistories", "provenance", "timeCalibrations", "version", "voices"
]);
const COLLECTION_ROOT_KEYS = new Set([
  ...
  "coMemoryResponses", "curatorAgent", "memoryInbox", "oralHistories", "provenance", "revisions", "revisits", "schemaVersion", "timeCalibrations", "version", "voices"
]);
const REDACTED_MEMORY_KEYS = new Set([
  "agentRunId", "attachments", "coverImage", "createdAt", "date", "emotionIntensity", "emotions",
  "entityRefs", "exhibitText", "favorite", "hall", "id", "importance", "location", "media",
  "mediaNote", "mediaSummary", "people", "rawContent", "schemaVersion", "sourceType", "tags", "title",
  "updatedAt", "voices", "voiceSummary"
]);
 ...
 */
function buildMediaArchive(options = {}) {
  return createArchive(buildMediaArchiveEntries(options, false));
}

/**
 * File-backed export path for production HTTP streaming. The returned cleanup
 * callback is idempotent and owns the private temporary directory.
 */
async function buildMediaArchiveFile(options = {}) {
  const outputRoot = options.outputRoot === undefined
    ? os.tmpdir()
    : requireAbsolutePath(options.outputRoot, "outputRoot");
  ensureRealDirectory(outputRoot, "outputRoot");
  let temporaryRoot = "";
  try {
    // Keep collection JSON and all DB-backed media metadata in one synchronous
    // snapshot. If this yielded before building entries, a same-tick deletion
    // could make collection.json retain a photo while the media descriptors
    // silently omitted it.
    temporaryRoot = fs.mkdtempSync(path.join(outputRoot, "time-isle-export-"));
    const entryRoot = path.join(temporaryRoot, "entries");
    const outputPath = path.join(temporaryRoot, "archive.time-isle");
    fs.mkdirSync(entryRoot, { mode: 0o700 });
    throwIfAborted(options.signal);
    const entries = buildMediaArchiveEntries(options, true, entryRoot);
    throwIfAborted(options.signal);
    const archive = await createArchiveFile(entries, { outputPath, signal: options.signal });
    let cleaned = false;
...
function parseExportMode(searchParams) {
  if (!searchParams || typeof searchParams.getAll !== "function") {
    throw mediaArchiveError("导出 mode 查询参数无效。", "MEDIA_ARCHIVE_EXPORT_MODE_INVALID");
  }
  const values = searchParams.getAll("mode");
  if (values.length === 0) return "full";
  if (values.length !== 1 || !["full", "redacted"].includes(values[0])) {
    throw mediaArchiveError("导出 mode 只能是 full 或 redacted，且不能重复。", "MEDIA_ARCHIVE_EXPORT_MODE_INVALID");
  }
  return values[0];
}

function buildMediaArchiveEntries(
  { collection, store, storage, voiceStorage, appVersion, schemaVersion, limits: suppliedLimits } = {},
  streamFiles,
  streamEntryRoot = null
) {
  ...
  normalizedCollection.version = version;
  normalizedCollection.schemaVersion = schema;
  normalizedCollection.mode = mode;
  normalizedCollection.exportedAt = exportedAt;
  validateCollectionRootKeys(normalizedCollection, schema);
  const featureSections = extractFeatureSectionsForWrite(normalizedCollection, mode, schema);
  validateRevisitIntentFeature(
    featureSections.find((section) => section.collectionKey === "revisitIntents")?.payload,
    [...collectionMemoryIds(normalizedCollection)]
  );
  const timeCalibrationSection = featureSections.find((section) => section.collectionKey === "timeCalibrations")?.payload;
  validateTimeCalibrationFeature(
    timeCalibrationSection,
    [...collectionMemoryIds(normalizedCollection)],
    timeCalibrationSection?.mode === "full" ? collectionEventIds(normalizedCollection) : [],
    timeCalibrationSection?.mode === "full" ? store?.validateTimeCalibrationBackup : undefined
  );
  const oralHistoryPayload = featureSections.find((section) => section.collectionKey === "oralHistories")?.payload;
  const oralVoiceAssetIds = oralHistoryPayload?.mode === "full"
    ? referencedOralVoiceAssetIds(oralHistoryPayload)
    : [];
  const voiceSection = buildVoiceArchiveSection({
    ...
    voiceStorage,
    validateVoiceBackup: schema >= 8 && Object.hasOwn(normalizedCollection, "voices") && typeof store?.validateVoiceBackup === "function"
      ? (payload, memoryIds) => store.validateVoiceBackup(payload, memoryIds, { additionalAssetIds: oralVoiceAssetIds })
      : undefined,
    streamFiles
  });
  validateOralHistoryFeature(
    oralHistoryPayload,
    oralHistoryPayload?.mode === "full" ? collectionEventIds(normalizedCollection) : [],
    voiceSection?.state?.mode === "full" ? voiceSection.state.assets.map((asset) => asset.id) : [],
    voiceSection?.state?.mode === "full" ? voiceSection.state.assets : undefined,
    oralHistoryPayload?.mode === "full" ? store?.validateOralHistoryBackup : undefined,
    mode
  );
  const curatorAgentPayload = featureSections.find((section) => section.collectionKey === "curatorAgent")?.payload;
  const exhibitionPayload = featureSections.find((section) => section.collectionKey === "exhibitions")?.payload;
  validateCuratorAgentFeature(
    curatorAgentPayload,
    [...collectionMemoryIds(normalizedCollection)],
    curatorAgentPayload?.mode === "full" ? collectionEventIds(normalizedCollection) : [],
    curatorAgentPayload?.mode === "full" ? featureExhibitionIds(exhibitionPayload) : [],
    curatorAgentPayload?.mode === "full" ? store?.validateCuratorAgentBackup : undefined,
    mode
  );
  const memoryInboxPayload = featureSections.find((section) => section.collectionKey === "memoryInbox")?.payload;
  validateMemoryInboxFeature(
    memoryInboxPayload,
    [...collectionMemoryIds(normalizedCollection)],
    memoryInboxPayload?.mode === "full" ? store?.validateMemoryInboxBackup : undefined,
    mode
  );
  validateProvenanceFeature(
    featureSections.find((section) => section.collectionKey === "provenance")?.payload,
    [...collectionMemoryIds(normalizedCollection)],
    mode
  );
  validateCoMemoryResponseFeature(
    featureSections.find((section) => section.collectionKey === "coMemoryResponses")?.payload,
    [...collectionMemoryIds(normalizedCollection)],
    mode === "full" ? store?.validateCoMemoryResponseBackup : undefined,
    mode
  );

  if (mode === "redacted") {
    const safeCollection = sanitizeRedactedCollection(normalizedCollection);
    return assembleArchiveEntries({
  ...
  const assets = selected.assets.map(({ asset, variants }) => {
    const archivedVariants = normalizeExportVariants(asset, variants, storage, imageEntries, streamFiles);
...
async function prepareMediaArchive(source, options = {}) {
  ...
  const supportedSchemaVersion = options.supportedSchemaVersion === undefined
    ? null
    : requireSchemaVersion(options.supportedSchemaVersion, "options.supportedSchemaVersion");
  ...
  try {
    ...
    const manifestFilePath = requiredExtractedPath(extractedByPath, stagingRoot, ARCHIVE_PATHS.manifest);
    const manifest = readJsonFile(manifestFilePath);
    const manifestEntries = validateManifest(manifest, extractedByPath, supportedSchemaVersion, limits);
    ...
    validateCollectionForImport(collection, manifest, limits);
    const featureSections = readFeatureSections(manifest, verifiedByPath, collection);
    validateRevisitIntentFeature(
      featureSections.revisitIntents,
      [...collectionMemoryIds(collection)]
    );
    const timeCalibrations = featureSections.timeCalibrations;
    validateTimeCalibrationFeature(
      timeCalibrations,
      [...collectionMemoryIds(collection)],
      timeCalibrations?.mode === "full" ? collectionEventIds(collection) : [],
      options.validateTimeCalibrationBackup
    );
    throwIfAborted(options.signal);
    const oralVoiceAssetIds = featureSections.oralHistories?.mode === "full"
      ? referencedOralVoiceAssetIds(featureSections.oralHistories)
      : [];
    const voiceSection = readVoiceArchiveSection({
      manifest,
      verifiedByPath,
      collection,
      validateVoiceBackup: typeof options.validateVoiceBackup === "function"
        ? (payload, memoryIds) => options.validateVoiceBackup(payload, memoryIds, { additionalAssetIds: oralVoiceAssetIds })
        : undefined
    });
    validateOralHistoryFeature(
      featureSections.oralHistories,
      featureSections.oralHistories?.mode === "full" ? collectionEventIds(collection) : [],
      voiceSection?.state?.mode === "full" ? voiceSection.state.assets.map((asset) => asset.id) : [],
      voiceSection?.state?.mode === "full" ? voiceSection.state.assets : undefined,
      options.validateOralHistoryBackup,
      manifest.mode
    );
    validateCuratorAgentFeature(
      featureSections.curatorAgent,
      [...collectionMemoryIds(collection)],
      featureSections.curatorAgent?.mode === "full" ? collectionEventIds(collection) : [],
      featureSections.curatorAgent?.mode === "full" ? featureExhibitionIds(featureSections.exhibitions) : [],
      options.validateCuratorAgentBackup,
      manifest.mode
    );
    validateMemoryInboxFeature(
      featureSections.memoryInbox,
      [...collectionMemoryIds(collection)],
      options.validateMemoryInboxBackup,
      manifest.mode
    );
    validateProvenanceFeature(
      featureSections.provenance,
      [...collectionMemoryIds(collection)],
      manifest.mode
    );
    validateCoMemoryResponseFeature(
      featureSections.coMemoryResponses,
      [...collectionMemoryIds(collection)],
      options.validateCoMemoryResponseBackup,
      manifest.mode
    );
...
function assembleArchiveEntries({
  collection,
  appVersion,
  schemaVersion,
  exportedAt,
  ...
  const entries = [jsonArchiveEntry(ARCHIVE_PATHS.collection, collection, streamEntryRoot)];
  if (media) {
    entries.push(
      jsonArchiveEntry(ARCHIVE_PATHS.assets, media.assets, streamEntryRoot),
      jsonArchiveEntry(ARCHIVE_PATHS.links, media.links, streamEntryRoot),
      jsonArchiveEntry(ARCHIVE_PATHS.observations, media.observations, streamEntryRoot),
      ...imageEntries
    );
  }
  for (const section of featureSections) {
    entries.push(jsonArchiveEntry(section.path, section.payload, streamEntryRoot));
  }
  if (voiceSection) {
    entries.push(
      jsonArchiveEntry(voiceSection.path, voiceSection.state, streamEntryRoot),
      ...voiceSection.audioEntries.map((entry) => ({
        path: entry.path,
        mime: entry.mime,
        ...(entry.filePath
          ? { filePath: entry.filePath, size: entry.byteSize, sha256: entry.sha256 }
          : { data: entry.data })
      }))
    );
  }
  ...
  const manifestEntry = jsonArchiveEntry(ARCHIVE_PATHS.manifest, manifest, streamEntryRoot);
  const archiveEntries = [
    { path: ARCHIVE_PATHS.manifest, ...archiveEntrySource(manifestEntry), mtime: 0 },
    ...entries.map((entry) => ({
      path: entry.path,
      mtime: 0,
...
function validateFeaturePayload(definition, payload, mode) {
  ...
  if (definition.customValidator === "oral-history") {
    return validateOralHistoryArchiveEnvelope(payload, mode);
  }
  if (definition.customValidator === "curator-agent") {
    return validateCuratorAgentArchiveEnvelope(payload, mode);
  }
  if (definition.customValidator === "memory-inbox") {
    validateMemoryInboxBackupPayload(payload);
    return payload.mode === "full" ? payload.items.length : payload.itemCount;
  }
  if (definition.customValidator === "provenance") {
    try {
      validateProvenanceBackupPayload(payload);
      return payload.mode === "full" ? payload.claims.length : payload.claimCount;
    } catch (cause) {
      throw mediaArchiveError("Provenance section is invalid.", "MEDIA_ARCHIVE_FEATURE_INVALID", cause);
    }
  }
  if (definition.customValidator === "co-memory-response") {
    try {
      validateCoMemoryResponseBackupPayload(payload);
      return payload.mode === "full" ? payload.responses.length : payload.responseCount;
    } catch (cause) {
      throw mediaArchiveError("Co-memory response section is invalid.", "MEDIA_ARCHIVE_FEATURE_INVALID", cause);
    }
  }
  if (mode === "full") {
...
function validateRevisitIntentFeature(payload, sourceMemoryIds) {
  if (payload === null || payload === undefined) return;
  try {
    validateRevisitIntentBackupPayload(payload, sourceMemoryIds);
  } catch (cause) {
    throw mediaArchiveError("回访意愿归档内容无效。", "MEDIA_ARCHIVE_FEATURE_INVALID", cause);
  }
}

function validateTimeCalibrationFeature(payload, sourceMemoryIds, sourceEventIds, validate) {
  if (payload === null || payload === undefined) return;
  if (payload.mode === "redacted-summary") {
    if (payload.uncertainCount + payload.alternativesCount > payload.calibrationCount) {
      throw mediaArchiveError("Time-calibration summary counts are inconsistent.", "MEDIA_ARCHIVE_FEATURE_INVALID");
    }
    return;
  }
  ...
  if (typeof validate !== "function") {
    throw mediaArchiveError(
      "Full time-calibration archives require an injected validator.",
      "MEDIA_ARCHIVE_TIME_CALIBRATION_VALIDATOR_REQUIRED"
    );
  }
  try {
    validate(payload, sourceMemoryIds, sourceEventIds);
  } catch (cause) {
    throw mediaArchiveError("Time-calibration archive content is invalid.", "MEDIA_ARCHIVE_FEATURE_INVALID", cause);
  }
}

function validateOralHistoryFeature(payload, eventIds, voiceAssetIds, voiceAssets, validate, mode) {
  if (payload === null || payload === undefined) return;
  validateOralHistoryArchiveState(payload, {
    mode,
    eventIds,
    voiceAssetIds,
    voiceAssets,
  ...
  if (payload.mode === "redacted-summary") assertRedactedOralHistoryPrivacy(payload);
}

function validateCuratorAgentFeature(payload, memoryIds, eventIds, exhibitionIds, validate, mode) {
  if (payload === null || payload === undefined) return;
  try {
    validateCuratorAgentArchiveState(payload, { mode, memoryIds, eventIds, exhibitionIds });
    if (payload.mode === "full") {
      ...
      if (result && typeof result.then === "function") {
        throw new TypeError("validateCuratorAgentBackup 必须同步执行。");
      }
    ...
    throw mediaArchiveError("受限策展归档内容无效。", "MEDIA_ARCHIVE_FEATURE_INVALID", cause);
  }
}

function validateMemoryInboxFeature(payload, memoryIds, validate, mode) {
  if (payload === null || payload === undefined) return;
  try {
    validateMemoryInboxBackupPayload(payload, payload.mode === "full" ? { memoryIds } : {});
    if (payload.mode === "full" && typeof validate === "function") {
      const result = validate(payload, memoryIds);
      if (result && typeof result.then === "function") throw new TypeError("validateMemoryInboxBackup 必须同步执行。");
      if (result !== true) throw new TypeError("记忆收件箱归档未通过业务校验。");
    }
    if (mode === "redacted" && payload.mode !== "redacted-summary") {
      throw new TypeError("脱敏归档不能携带完整记忆收件箱来源回执。");
    }
  } catch (cause) {
    if (cause?.code === "MEDIA_ARCHIVE_FEATURE_INVALID") throw cause;
    throw mediaArchiveError("记忆收件箱归档内容无效。", "MEDIA_ARCHIVE_FEATURE_INVALID", cause);
  }
}

function validateProvenanceFeature(payload, memoryIds, mode) {
  if (payload === null || payload === undefined) return;
  try {
    if (mode === "full" && payload.mode !== "full") {
      throw new TypeError("Full archives require full provenance state.");
    }
    if (mode === "redacted" && payload.mode !== "redacted-summary") {
      throw new TypeError("Redacted archives may contain only provenance counts.");
    }
    validateProvenanceBackupPayload(payload, payload.mode === "full" ? { memoryIds } : {});
  } catch (cause) {
    throw mediaArchiveError("Provenance archive content is invalid.", "MEDIA_ARCHIVE_FEATURE_INVALID", cause);
  }
}

function validateCoMemoryResponseFeature(payload, memoryIds, validate, mode) {
  if (payload === null || payload === undefined) return;
  try {
    if (mode === "full" && payload.mode !== "full") {
      throw new TypeError("Full archives require full co-memory response state.");
    }
    if (mode === "redacted" && payload.mode !== "redacted-summary") {
      throw new TypeError("Redacted archives may contain only co-memory response counts.");
    }
    validateCoMemoryResponseBackupPayload(payload, payload.mode === "full" ? { memoryIds } : {});
    if (payload.mode === "full" && typeof validate === "function") {
      const result = validate(payload, memoryIds);
      if (result && typeof result.then === "function") {
        throw new TypeError("validateCoMemoryResponseBackup must run synchronously.");
      }
      if (result !== true) throw new TypeError("Co-memory response archive failed business validation.");
    }
  } catch (cause) {
    throw mediaArchiveError("Co-memory response archive content is invalid.", "MEDIA_ARCHIVE_FEATURE_INVALID", cause);
  }
}
...
function normalizeExportVariants(asset, variants, storage, imageEntries, streamFiles = false) {
  ...
  for (const variant of variants) {
    ...
    const contentHash = requireSha256(variant.sha256, "variant.sha256");
    imageEntries.push({
      path: archivePath,
      mime: mimeType,
      ...(streamFiles
        ? { filePath, size: data.length, sha256: contentHash }
        : { data })
    });
    byKind.set(kind, {
      assetId: asset.id,
...
function validateManifest(manifest, extractedByPath, supportedSchemaVersion, limits) {
  ...
  const schemaVersion = requireSchemaVersion(manifest.schemaVersion, "manifest.schemaVersion");
  requireTimestamp(manifest.exportedAt, "manifest.exportedAt");
  if (schemaVersion >= CURATOR_AGENT_SCHEMA_VERSION && manifest.formatVersion !== ARCHIVE_FORMAT_VERSION) {
    throw mediaArchiveError(
      "Schema 14 归档必须使用 V2 section 清单携带受限策展状态。",
      "MEDIA_ARCHIVE_REQUIRED_SECTION_MISSING"
    );
  }
  if (supportedSchemaVersion !== null && schemaVersion > supportedSchemaVersion) {
    throw mediaArchiveError(
      `归档 schema ${schemaVersion} 高于当前支持的 schema ${supportedSchemaVersion}。`,
  ...
  if (manifest.entries.length + 1 > limits.maxEntries) {
...
function verifyManifestEntries(manifestEntries, extractedByPath, stagingRoot) {
  ...
  for (const descriptor of manifestEntries) {
    ...
    const filePath = extracted.filePath || resolveStagingFile(stagingRoot, descriptor.path);
...
function validateCollectionForImport(collection, manifest, limits) {
  assertCollection(collection);
  validateCollectionRootKeys(collection, manifest.schemaVersion);
  assertCollectionRecordLimit(collection, limits);
  validateCollectionMetadata(manifest, collection);
  collectionMemoryIds(collection);
}

function validateCollectionRootKeys(collection, schemaVersion) {
  const unknown = Object.keys(collection).filter((key) => !COLLECTION_ROOT_KEYS.has(key));
  if (unknown.length) {
    throw mediaArchiveError(
      `馆藏根对象包含当前 schema 不支持的字段：${unknown.join(", ")}。`,
  ...
  for (const [key, since] of gated) {
    if (Object.hasOwn(collection, key) && schemaVersion < since) {
      throw mediaArchiveError(
        `${key} 数据不能由 schema ${schemaVersion} 归档声明。`,
        "MEDIA_ARCHIVE_FEATURE_SCHEMA_INVALID"
      );
    }
  }
...
function validateCollectionMetadata(manifest, collection) {
  // Format V1 did not make collection-level duplicate metadata contractual.
  // Preserve only absence compatibility; any field that is present must still
  // be valid and exactly equal to its authoritative manifest field.
  const legacy = manifest.formatVersion === LEGACY_ARCHIVE_FORMAT_VERSION;
  const fields = [
    ["schemaVersion", "schemaVersion", requireSchemaVersion],
    ["mode", "mode", requireArchiveMode],
    ["version", "appVersion", requireVersion],
    ["exportedAt", "exportedAt", requireTimestamp]
  ];

  for (const [collectionKey, manifestKey, validate] of fields) {
    if (!Object.hasOwn(collection, collectionKey)) {
      if (legacy) continue;
      throw mediaArchiveError(
        `collection.${collectionKey} 缺失，无法核对归档元数据。`,
        "MEDIA_ARCHIVE_METADATA_MISMATCH"
      );
    }
    validate(collection[collectionKey], `collection.${collectionKey}`);
    if (!Object.is(collection[collectionKey], manifest[manifestKey])) {
      throw mediaArchiveError(
        `collection.${collectionKey} 与 manifest.${manifestKey} 不一致。`,
        "MEDIA_ARCHIVE_METADATA_MISMATCH"
      );
    }
  }
...
function validateRedactedArchive(manifest, manifestEntries, collection, voiceSection) {
  validateRedactedCollectionPrivacy(collection);
...
function buildPreparedDescriptor({ stagingRoot, manifest, collection, assets, links, observations, featureSections, voiceSection, manifestFilePath, verifiedByPath }) {
  ...
  return {
    ...
    revisions: featureSections.revisions || null,
    revisitIntents: featureSections.revisitIntents || null,
    timeCalibrations: featureSections.timeCalibrations || null,
    curatorAgent: featureSections.curatorAgent || null,
    memoryInbox: featureSections.memoryInbox || null,
    provenance: featureSections.provenance || null,
    coMemoryResponses: featureSections.coMemoryResponses || null,
    ...
    files: {
      ...
      collection: requiredVerifiedPath(verifiedByPath, ARCHIVE_PATHS.collection),
      assets: manifest.mode === "full" ? requiredVerifiedPath(verifiedByPath, ARCHIVE_PATHS.assets) : null,
      links: manifest.mode === "full" ? requiredVerifiedPath(verifiedByPath, ARCHIVE_PATHS.links) : null,
      mediaObservations: manifest.mode === "full"
        ? requiredVerifiedPath(verifiedByPath, ARCHIVE_PATHS.observations)
        : null,
      variants,
...
function sanitizeRedactedCollection(collection) {
  ...
  for (const key of Object.keys(safe)) {
    if (!REDACTED_COLLECTION_KEYS.has(key)) delete safe[key];
  }
  safe.mode = "redacted";
  safe.product = "时屿";
  safe.productEnglish = "TIME ISLE";
  safe.privacy = REDACTED_COLLECTION_PRIVACY;
  safe.memories = safe.memories.map(redactMemory);
  safe.count = safe.memories.length;
  if (safe.archaeology !== undefined) {
    try { safe.archaeology = safe.archaeology?.mode === "redacted-summary" ? cloneJson(safe.archaeology) : redactArchaeologyBackup(safe.archaeology); }
    catch (cause) {
      throw mediaArchiveError("脱敏记忆考古摘要无效。", "MEDIA_ARCHIVE_REDACTED_PRIVACY_INVALID", cause);
    }
  }
  return safe;
}

function validateRedactedCollectionPrivacy(collection) {
  const unknownCollectionKeys = Object.keys(collection).filter((key) => !REDACTED_COLLECTION_KEYS.has(key));
  if (unknownCollectionKeys.length || collection.mode !== "redacted" || collection.privacy !== REDACTED_COLLECTION_PRIVACY ||
      collection.product !== "时屿" || collection.productEnglish !== "TIME ISLE" ||
      collection.count !== collection.memories.length) {
    throw mediaArchiveError("脱敏馆藏边界声明无效或包含未知字段。", "MEDIA_ARCHIVE_REDACTED_PRIVACY_INVALID");
  }
  if (collection.archaeology !== undefined) {
    try { validateArchaeologyBackup(collection.archaeology, []); }
    catch (cause) {
      throw mediaArchiveError("脱敏记忆考古摘要包含不允许的内容。", "MEDIA_ARCHIVE_REDACTED_PRIVACY_INVALID", cause);
    }
  }
  collection.memories.forEach((memory, index) => validateRedactedMemory(memory, index));
}

function validateRedactedMemory(memory, index) {
  assertPlainObject(memory, `collection.memories[${index}]`);
  const unknownKeys = Object.keys(memory).filter((key) => !REDACTED_MEMORY_KEYS.has(key));
  const people = Array.isArray(memory.people) ? memory.people : [];
  const media = Array.isArray(memory.media) ? memory.media : [];
  const voices = Array.isArray(memory.voices) ? memory.voices : [];
  const attachments = Array.isArray(memory.attachments) ? memory.attachments : [];
  const entityRefs = Array.isArray(memory.entityRefs) ? memory.entityRefs : [];
  if (unknownKeys.length || memory.rawContent !== "[已隐藏原始记忆]" ||
      (people.length && (people.length !== 1 || people[0] !== "[已隐藏人物]")) ||
      !["", "[已隐藏地点]"].includes(memory.location || "") ||
      !["", "[已隐藏媒体备注]"].includes(memory.mediaNote || "") ||
      memory.coverImage || memory.agentRunId || media.length || voices.length || attachments.length || entityRefs.length) {
    throw mediaArchiveError("脱敏展品仍包含正文、人物、实体、媒体、声音或内部引用。", "MEDIA_ARCHIVE_REDACTED_PRIVACY_INVALID");
  }
  validateRedactedMediaSummary(memory.mediaSummary);
  validateRedactedVoiceSummary(memory.voiceSummary);
}

function validateRedactedMediaSummary(summary) {
  assertPlainObject(summary, "redacted memory.mediaSummary");
  assertExactObjectKeys(summary, ["count", "coverAssetId", "coverThumbnailUrl"], "redacted memory.mediaSummary");
  if (!Number.isSafeInteger(summary.count) || summary.count < 0 || summary.coverAssetId !== "" || summary.coverThumbnailUrl !== "") {
    throw mediaArchiveError("脱敏图片摘要包含内部引用。", "MEDIA_ARCHIVE_REDACTED_PRIVACY_INVALID");
  }
}

function validateRedactedVoiceSummary(summary) {
  assertPlainObject(summary, "redacted memory.voiceSummary");
  assertExactObjectKeys(summary, ["count", "confirmedTranscriptCount"], "redacted memory.voiceSummary");
  if (!Number.isSafeInteger(summary.count) || summary.count < 0 || summary.confirmedTranscriptCount !== 0) {
    throw mediaArchiveError("脱敏声音摘要包含文字稿信息。", "MEDIA_ARCHIVE_REDACTED_PRIVACY_INVALID");
  }
}
...
function collectionEventIds(collection) {
  const archaeology = collection?.archaeology;
  if (!archaeology || archaeology.mode !== "full") return [];
  if (!Array.isArray(archaeology.events)) {
    throw mediaArchiveError("Full archaeology data must expose an events array.", "MEDIA_ARCHIVE_FEATURE_INVALID");
  }
  const ids = new Set();
  for (const event of archaeology.events) {
    assertPlainObject(event, "archaeology event");
    const id = requireId(event.id, "archaeology event.id");
    if (ids.has(id)) {
      throw mediaArchiveError("Archaeology event IDs must be unique.", "MEDIA_ARCHIVE_FEATURE_INVALID");
    }
    ids.add(id);
  }
  return [...ids];
...
function featureExhibitionIds(backup) {
  if (!backup || backup.mode !== "full" || !Array.isArray(backup.exhibitions)) return [];
  const ids = new Set();
  for (const exhibition of backup.exhibitions) {
    assertPlainObject(exhibition, "exhibition");
    const id = requireId(exhibition.id, "exhibition.id");
    if (ids.has(id)) throw mediaArchiveError("主题展览包含重复 ID。", "MEDIA_ARCHIVE_FEATURE_INVALID");
    ids.add(id);
  }
  return [...ids];
}
...
function jsonArchiveEntry(archivePath, value, streamEntryRoot) {
  const data = jsonBuffer(value);
  if (!streamEntryRoot) return { path: archivePath, mime: JSON_MIME, data };
  const fileName = `${crypto.createHash("sha256").update(archivePath).digest("hex")}.json`;
  const filePath = path.join(streamEntryRoot, fileName);
  try {
    fs.writeFileSync(filePath, data, { flag: "wx", mode: 0o600 });
  } catch (cause) {
    throw mediaArchiveError(`归档 JSON 临时文件无法安全写入：${archivePath}`, "MEDIA_ARCHIVE_STAGING_INVALID", cause);
  }
  return {
    path: archivePath,
    mime: JSON_MIME,
...
function archiveEntrySource(entry) {
  return entry.filePath
    ? { filePath: entry.filePath, size: entry.size, sha256: entry.sha256 }
    : { data: entry.data };
}

function describeEntry(entry) {
  if (entry.filePath) {
    const measured = hashRegularFile(entry.filePath, entry.path);
    if (measured.bytes !== entry.size || measured.sha256 !== entry.sha256) {
      throw mediaArchiveError(`归档源文件在打包前发生变化：${entry.path}`, "MEDIA_ARCHIVE_HASH_MISMATCH");
    }
    return {
      path: entry.path,
...
function assertArchiveEntryLimits(entries, limits) {
  ...
  for (const entry of entries) {
    const bytes = entry.filePath ? entry.size : entry.data?.length;
    if (!Number.isSafeInteger(bytes) || bytes < 0 ||
        (!entry.filePath && !Buffer.isBuffer(entry.data)) || bytes > limits.maxEntryBytes) {
...
function requiredExtractedPath(extractedByPath, stagingRoot, archivePath) {
  const entry = extractedByPath.get(archivePath);
  if (!entry) {
    throw mediaArchiveError(`归档缺少 ${archivePath}。`, "MEDIA_ARCHIVE_ENTRY_MISSING");
  }
  return entry.filePath || resolveStagingFile(stagingRoot, archivePath);
}
...
function requireArchiveMode(value, name) {
  return requireEnum(value, new Set(["full", "redacted"]), name);
}
...
function ensureRealDirectory(directoryPath, name) {
  try {
    fs.mkdirSync(directoryPath, { recursive: true, mode: 0o700 });
    const stat = fs.lstatSync(directoryPath);
    if (!stat.isDirectory() || stat.isSymbolicLink()) throw new Error("not a real directory");
  } catch (cause) {
    throw mediaArchiveError(`${name} 必须是安全的真实目录。`, "MEDIA_ARCHIVE_STAGING_INVALID", cause);
  }
}
...
function hashRegularFile(filePath, label) {
  ...
  try {
    ...
    if (!opened.isFile() || opened.size !== before.size) throw new Error("file changed while opening");
    ...
    if (bytes !== opened.size) throw new Error("file changed while hashing");
    return { bytes, sha256: hash.digest("hex") };
  } catch (cause) {
    if (String(cause?.code || "").startsWith("MEDIA_ARCHIVE_")) throw cause;
    throw mediaArchiveError(`归档源文件不可安全读取：${label}`, "MEDIA_ARCHIVE_FILE_MISSING", cause);
  } finally {
