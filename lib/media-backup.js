...
async function buildMediaArchiveFile(options = {}) {
  ...
  const temporaryRoot = await fs.promises.mkdtemp(path.join(outputRoot, "time-isle-export-"));
...
async function prepareMediaArchive(source, options = {}) {
  ...
  try {
    ...
    const manifestEntries = validateManifest(manifest, extractedByPath, supportedSchemaVersion);
...
function validateManifest(manifest, extractedByPath, supportedSchemaVersion) {
