...
function initializeCuratorAgentDatabase(options = {}) {
  ...
  statements.interruptRunning.run(interruptedAt, interruptedAt);
  ...
  function createCuratorAgentRun(input = {}, mutation = {}) {
    ...
    if (duplicate) {
      ...
      return { created: false, idempotent: true, workspace: getCuratorAgentRunWorkspace(duplicate.id) };
    }
    ...
    const id = newId("curator-run");
    runAtomic(() => statements.insertRun.run(runInsertParameters({
      id,
      idempotencyKey,
      requestSha256,
      request,
    ...
    return { created: true, idempotent: false, workspace: getCuratorAgentRunWorkspace(id) };
  }
  ...
  function executeCuratorAgentRun(runId, mutation = {}) {
    ...
    const startedAt = requireTimestamp(now());
    runAtomic(() => {
    ...
    try {
        ...
        onStep: (step) => {
          ...
          runAtomic(() => {
            ...
            statements.updateRunningUsage.run(step.position + 1, step.position + 1,
              Number(current.result_bytes) + step.resultBytes, id);
          });
        }
      });
      ...
      if (current && !current.deleted_at && current.status === "running") {
        const failedAt = requireTimestamp(now());
        ...
        runAtomic(() => statements.failRun.run(
          failedAt,
          failedAt,
          code,
          message,
  ...
  function decideCuratorAgentRun(runId, input = {}, mutation = {}) {
    ...
    if (decisionInput.decision === "approve") {
      ...
      if (stale) {
        const timestamp = requireTimestamp(now());
        runAtomic(() => statements.markNeedsReview.run(timestamp, id));
  ...
  function purgeCuratorAgentRunsForMemory(memoryId) {
    ...
    const runIds = statements.runIdsForMemory.all(id, id, id, id).map((row) => row.id);
    return runAtomic(() => {
  ...
  function purgeCuratorAgentRunsForExhibition(exhibitionId) {
    ...
    const runIds = statements.runIdsForExhibition.all(id, id, id, id).map((row) => row.id);
    return runAtomic(() => {
      for (const runId of runIds) statements.hardDeleteRun.run(runId);
      return { runsDeleted: runIds.length, runIds };
    });
  ...
  function buildCuratorAgentBackup(mode = "full") {
    ...
    const state = {
      ...
      runs: listCuratorAgentRuns({ limit: MAX_RUNS }).map((run) => toArchiveWorkspace(getCuratorAgentRunWorkspace(run.id)))
    };
...
function prepareStatements(db) {
  return {
    interruptRunning: db.prepare(`
      UPDATE curator_agent_runs SET status = 'interrupted', interrupted_at = ?, updated_at = ?,
    ...
    decisionsForRun: db.prepare("SELECT * FROM curator_agent_decisions WHERE run_id = ? ORDER BY datetime(created_at), id"),
    ...
    runIdsForMemory: db.prepare(`
      ...
      WHERE run.deleted_at = '' AND (
        EXISTS (
          ...
          WHERE json_extract(source.value, '$.memoryId') = ?
        ) OR EXISTS (
          SELECT 1 FROM curator_agent_steps step, json_tree(step.args_json) reference
          WHERE step.run_id = run.id AND reference.type = 'text' AND reference.atom = ?
        ) OR EXISTS (
          SELECT 1 FROM curator_agent_steps step, json_tree(step.result_json) reference
          WHERE step.run_id = run.id AND reference.type = 'text' AND reference.atom = ?
        )
      )
      ORDER BY run.id
    ...
    runIdsForExhibition: db.prepare(`
      ...
      WHERE run.deleted_at = '' AND (
        EXISTS (
          ...
          WHERE json_extract(duplicate.value, '$.id') = ?
        ) OR EXISTS (
          SELECT 1 FROM curator_agent_steps step, json_tree(step.args_json) reference
          WHERE step.run_id = run.id AND reference.type = 'text' AND reference.atom = ?
        ) OR EXISTS (
          ...
          WHERE decision.run_id = run.id AND json_extract(decision.outcome_json, '$.exhibitionId') = ?
        )
      )
      ORDER BY run.id
    `),
...
function requireTimestamp(value) {
  ...
  if (!timestamp || Number.isNaN(Date.parse(timestamp))) throw new TypeError("Curator-agent timestamp is invalid.");
