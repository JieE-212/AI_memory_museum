...
function parseWebmOpus(buffer) {
  ...
  const tracks = [];
  const clusters = [];
  walkSegmentEbml(buffer, segment.dataStart, segment.dataEnd, (element) => {
...
function walkSegmentEbml(buffer, start, end, visitor) {
  let offset = start;
  let count = 0;
  while (offset < end) {
    if (++count > MAX_PARSED_ELEMENTS) invalid("WebM 元素数量过多。", "VOICE_WEBM_INVALID");
    let element = readEbmlElement(buffer, offset, end, EBML_IDS.CLUSTER);
    if (element.id === EBML_IDS.CLUSTER && element.unknownSize) {
      element = { ...element, dataEnd: findUnknownClusterEnd(buffer, element.dataStart, end) };
    }
    visitor(element);
    offset = element.dataEnd;
  }
...
function findUnknownClusterEnd(buffer, start, end) {
  let offset = start;
  let count = 0;
  while (offset < end) {
    if (++count > MAX_PARSED_ELEMENTS) invalid("WebM Cluster 元素数量过多。", "VOICE_WEBM_INVALID");
    const child = readEbmlElement(buffer, offset, end, EBML_IDS.CLUSTER);
    if (child.id === EBML_IDS.CLUSTER) return offset;
...
function readEbmlElement(buffer, offset, limit, allowUnknownSize) {
  ...
  const elementId = Number(id.value);
  const allowedUnknownId = allowUnknownSize === true ? EBML_IDS.SEGMENT : Number(allowUnknownSize) || 0;
  if (size.unknown && elementId !== allowedUnknownId) invalid(`WebM 元素 0x${elementId.toString(16)} 不能使用未知长度。`, "VOICE_WEBM_INVALID");
  ...
  return { id: elementId, start: offset, dataStart, dataEnd, unknownSize: size.unknown };
}
