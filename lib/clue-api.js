...
function createClueApi(options = {}) {
  ...
  const projectMemory = typeof options.projectMemory === "function" ? options.projectMemory : (memory) => memory;
  ...
  async function handleSearch(request, response, url) {
    ...
    const hall = normalizeSearchHall(url.searchParams.get("hall"));
    const sort = normalizeSearchSort(url.searchParams.get("sort"));
    const effectiveSort = sort || "relevance";
    const cursorContext = { query, hall, sort: effectiveSort };
    const cursor = decodeSearchCursor(url.searchParams.get("cursor"), cursorContext);
    const paginated = Boolean(hall || sort || url.searchParams.get("cursor"));
    ...
    const raw = await Promise.resolve(search.call(store, query, {
      limit: paginated ? 5000 : limit,
      allowExpandedLimit: paginated,
      includeMeta: true,
      mode: "clue"
    }));
    const sourceResults = Array.isArray(raw) ? raw : Array.isArray(raw?.results) ? raw.results : [];
    const filteredResults = sortSearchResults(
      sourceResults.filter((item) => !hall || normalizeSearchResult(item).memory?.hall === hall),
      effectiveSort
    );
    const remainingResults = cursor
      ? filteredResults.filter((item) => compareSearchCursorKey(searchCursorKey(item, effectiveSort), cursor, effectiveSort) > 0)
      : filteredResults;
    const pageResults = remainingResults.slice(0, limit);
    const results = await Promise.all(pageResults.map(async (item) => {
      let normalized = normalizeSearchResult(item);
      const decoratedMemory = await Promise.resolve(decorateMemory(normalized.memory));
      normalized = {
        ...normalized,
        memory: projectMemory(decoratedMemory || normalized.memory),
    ...
    return sendJson(response, 200, {
      ...
      nextCursor: pageResults.length && remainingResults.length > pageResults.length
        ? encodeSearchCursor(pageResults.at(-1), cursorContext)
        : null,
      results,
      engine: {
        mode: "clue",
  ...
  function normalizeSearchHall(value) {
    const hall = String(value || "").trim();
    if (!hall || hall === "all") return "";
    if (!/^[a-zA-Z0-9_-]{1,80}$/u.test(hall)) throw apiError(400, "CLUE_HALL_INVALID", "展厅筛选值无效。");
    return hall;
  }
  ...
  function normalizeSearchSort(value) {
    ...
    if (!["recent", "oldest", "importance", "title", "relevance"].includes(sort)) throw apiError(400, "CLUE_SORT_INVALID", "排序方式无效。");
    return sort;
  }

  function sortSearchResults(results, sort) {
    return [...results].sort((left, right) => (
      compareSearchCursorKey(searchCursorKey(left, sort), searchCursorKey(right, sort), sort)
    ));
  }

  function searchMemoryId(memory) {
    return String(memory?.id || memory?.memoryId || "");
  }

  function searchTimestamp(memory) {
    const timestamp = Date.parse(memory.date || memory.createdAt || memory.updatedAt || "");
    return Number.isFinite(timestamp) ? timestamp : 0;
  }

  function searchCursorKey(item, sort) {
    const normalized = normalizeSearchResult(item);
    const memory = normalized.memory || {};
    const base = { id: searchMemoryId(memory) };
    if (sort === "relevance") return { ...base, score: normalized.score, directEvidenceCount: finiteNumber(item?.directEvidenceCount ?? normalized.directEvidenceCount ?? 0) };
    if (sort === "importance") return { ...base, importance: finiteNumber(memory.importance), timestamp: searchTimestamp(memory) };
    if (sort === "title") return { ...base, title: String(memory.title || "") };
    return { ...base, timestamp: searchTimestamp(memory) };
  }

  function compareSearchCursorKey(left, right, sort) {
    if (sort === "relevance") return right.score - left.score || right.directEvidenceCount - left.directEvidenceCount || left.id.localeCompare(right.id, "en");
    if (sort === "oldest") return left.timestamp - right.timestamp || left.id.localeCompare(right.id, "en");
    if (sort === "importance") return right.importance - left.importance || right.timestamp - left.timestamp || left.id.localeCompare(right.id, "en");
    if (sort === "title") return left.title.localeCompare(right.title, "zh-CN") || left.id.localeCompare(right.id, "en");
    return right.timestamp - left.timestamp || left.id.localeCompare(right.id, "en");
  }

  function encodeSearchCursor(item, context) {
    return Buffer.from(JSON.stringify({ v: 2, ...context, key: searchCursorKey(item, context.sort) }), "utf8").toString("base64url");
  }

  function decodeSearchCursor(value, context) {
    ...
    try {
      ...
      if (payload?.v !== 2 || payload.query !== context.query || payload.hall !== context.hall || payload.sort !== context.sort || !isValidSearchCursorKey(payload.key, context.sort)) {
        throw new Error("cursor context mismatch");
      }
      return payload.key;
    } catch {
      throw apiError(400, "CLUE_CURSOR_INVALID", "cursor 无效、已经过期或不属于当前检索条件。");
    }
  }

  function isValidSearchCursorKey(key, sort) {
    if (!key || typeof key !== "object" || Array.isArray(key) || !isValidSearchCursorId(key.id)) return false;
    if (sort === "relevance") return Number.isFinite(key.score) && Number.isSafeInteger(key.directEvidenceCount) && key.directEvidenceCount >= 0;
  ...
  function isValidSearchCursorId(value) {
