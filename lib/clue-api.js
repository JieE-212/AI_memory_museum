...
function createClueApi(options = {}) {
  ...
  async function handleSearch(request, response, url) {
    ...
    const cursorContext = { query, hall, sort: sort || "relevance" };
    const offset = decodeSearchCursor(url.searchParams.get("cursor"), cursorContext);
    ...
    return sendJson(response, 200, {
      ...
      nextCursor: offset + results.length < filteredResults.length ? encodeSearchCursor(offset + results.length, cursorContext) : null,
      results,
      engine: {
        mode: "clue",
        label: "字段与线索检索",
  ...
  function normalizeSearchSort(value) {
    ...
    if (!["recent", "oldest", "importance", "title"].includes(sort)) throw apiError(400, "CLUE_SORT_INVALID", "排序方式无效。");
    return sort;
  }

  function sortSearchResults(results, sort) {
    if (!sort) return [...results];
    return results.map((item, index) => ({ item, index, memory: normalizeSearchResult(item).memory || {} }))
      .sort((left, right) => {
        if (sort === "oldest") return searchTimestamp(left.memory) - searchTimestamp(right.memory) || searchMemoryId(left.memory).localeCompare(searchMemoryId(right.memory), "en") || left.index - right.index;
        if (sort === "importance") return Number(right.memory.importance) - Number(left.memory.importance) || searchTimestamp(right.memory) - searchTimestamp(left.memory) || searchMemoryId(left.memory).localeCompare(searchMemoryId(right.memory), "en") || left.index - right.index;
        if (sort === "title") return String(left.memory.title || "").localeCompare(String(right.memory.title || ""), "zh-CN") || searchMemoryId(left.memory).localeCompare(searchMemoryId(right.memory), "en") || left.index - right.index;
        return searchTimestamp(right.memory) - searchTimestamp(left.memory) || searchMemoryId(left.memory).localeCompare(searchMemoryId(right.memory), "en") || left.index - right.index;
      })
  ...
  function encodeSearchCursor(offset, context) {
    return Buffer.from(JSON.stringify({ v: 1, ...context, offset }), "utf8").toString("base64url");
  }
  ...
  function decodeSearchCursor(value, context) {
    ...
    try {
      ...
      if (payload?.v !== 1 || payload.query !== context.query || payload.hall !== context.hall || payload.sort !== context.sort) {
