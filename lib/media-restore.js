...
function restorePreparedArchive(options = {}) {
  ...
  if (voices && typeof validateVoiceBackup === "function") {
    validateFeatureBackup("声音", voices, sourceIds, validateVoiceBackup);
  }
  ...
  const sourceEventIds = timeCalibrations?.mode === "full" ? sourceArchaeologyEventIds(archaeology) : [];
  ...
  try {
    ...
    store.withTransaction(() => {
      ...
      if (timeCalibrations?.mode === "full" && typeof restoreTimeCalibrationBackup === "function") {
        const eventIdMap = requireCompleteEventIdMap(archaeologyResult?.idMap?.events, sourceEventIds);
