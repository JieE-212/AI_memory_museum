...
function verifyPreparedVoiceFile(source, file, stagingRoot) {
  ...
  const expectedFilePath = path.resolve(stagingRoot, ...expectedArchivePath.split("/"));
  if (file.archivePath !== expectedArchivePath || path.resolve(file.filePath) !== expectedFilePath) {
