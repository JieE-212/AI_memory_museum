...
function restorePreparedArchive(options = {}) {
  ...
  throwIfAborted(signal);
  const memoryPlan = buildMemoryPlan(sourceMemories, store, normalizeMemory, createId, signal);
  assertCoMemoryIdentityMapping(coMemoryResponses, memoryPlan.idMap);
  const assetPlan = buildAssetPlan(prepared, store, storage, createId, signal);
  const linksByMemory = mapLinks(prepared.links, memoryPlan.idMap, assetPlan.idMap, signal);
  ...
  try {
    throwIfAborted(signal);
    materializeAssetFiles(assetPlan.newAssets, movedDirectories, signal);
    materializeVoiceFiles(voicePlan, createdVoiceFiles, createdVoiceDirectories, voiceStorage, signal);
    throwIfAborted(signal);
    ...
    const result = store.withTransaction(() => {
      // Abort is cooperative between synchronous SQLite calls. A single call
      // already executing cannot be interrupted, but the transaction rolls
      // back at the next checkpoint.
      throwIfAborted(signal);
      ...
      for (const observation of observationPlan) {
        throwIfAborted(signal);
        store.saveMediaObservation(observation);
        throwIfAborted(signal);
      }
      ...
      if (voices?.mode === "full" && typeof restoreVoiceBackup === "function") {
        ...
        voiceResult = restoreVoiceBackup(voices, {
          ...
          notifyConfirmedTranscriptChanges: entities?.mode !== "full",
...
function buildVoiceRestorePlan({ prepared, voices, store, voiceStorage, memoryIdMap, createId, signal }) {
  throwIfAborted(signal);
  const empty = {
    idMap: new Map(),
  ...
  const existingAssets = listAllVoiceAssets(store, signal);
...
function listAllVoiceAssets(store, signal) {
  const output = [];
  let offset = 0;
  while (true) {
    throwIfAborted(signal);
...
function materializeVoiceFiles(plan, createdFiles, createdDirectories, voiceStorage, signal) {
...
function buildMemoryPlan(sourceMemories, store, normalizeMemory, createId, signal) {
  throwIfAborted(signal);
...
function buildAssetPlan(prepared, store, storage, createId, signal) {
  const existingAssets = listAllAssets(store, signal);
...
function mapLinks(sourceLinks, memoryIdMap, assetIdMap, signal) {
...
function buildObservationPlan(sourceObservations, memoryIdMap, assetIdMap, sourcePrivacyModes, store, createId, signal) {
...
function materializeAssetFiles(plans, movedDirectories, signal) {
...
function listAllAssets(store, signal) {
  const assets = [];
  let offset = 0;
  while (true) {
    throwIfAborted(signal);
...
function throwIfAborted(signal) {
  ...
  const error = new Error("Media restore was aborted.");
