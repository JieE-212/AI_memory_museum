...
function initializeExhibitionDatabase(options = {}) {
  ...
  function validateExhibitionBackup(backup, sourceMemoryIds) {
    ...
    if (backup.mode === "redacted-summary") {
      const expectedKeys = ["exhibitionCount", "mode", "note", "publishedCount"];
      const keys = Object.keys(backup).sort();
      if (keys.length !== expectedKeys.length || keys.some((key, index) => key !== expectedKeys[index]) ||
          !Number.isSafeInteger(backup.exhibitionCount) || backup.exhibitionCount < 0 || backup.exhibitionCount > LIMITS.exhibitions ||
          !Number.isSafeInteger(backup.publishedCount) || backup.publishedCount < 0 || backup.publishedCount > backup.exhibitionCount ||
          backup.note !== "展览叙事、成员和原文引用已从脱敏备份中移除。") {
        throw exhibitionError("脱敏主题展览摘要无效。", "EXHIBITION_BACKUP_INVALID", 400);
      }
      return true;
    }
