...
function createRuntimeMetadata(options = {}) {
  ...
  function demoStatus() {
    return {
      ...
      curatorAgentMode: interviewDemo ? "read-only-synthetic-sample" : "deterministic-curation-workflow",
      ...
      semanticRecallMode: interviewDemo ? "device-worker-read-only-demo" : "device-worker-optional-local-derived-cache",
      ...
      aiMode: interviewDemo ? "public-mock" : aiEnabled ? "external-model-configured" : "local-rules",
  ...
  function version() {
    return {
      ...
      productFlow: ["先保存原文", "照片与声音随后关联", "可选整理并更新同一件展品", "字段与线索检索及讲解", "设备内按意思找回", "主题策展", "确定性策展工作流", "共忆见证", "设备内可解释镜片", "多视角记忆对照", "记忆回访", "时光胶囊与加密分享", "记忆考古", "口述史回答与时间来源", "历史恢复", "锁馆与一次性真实恢复演练", "安全导出"],
      v7: {
      ...
      v10: {
        deterministicWorkflow: "固定四项本地只读工具和确定性规则，不是自治 Agent、模型规划器或后台自主执行器",
        boundedAgent: "历史字段名保留兼容；六步/四次读取/两秒预算，不访问网络、文件、任意 SQL，也不把记忆正文解释成指令",
        humanDecisions: "策展工作流只生成绑定来源快照的提案；保存草稿、确认关联与发布分别需要一次独立人工决定，分享只交接现有隐私编辑台",
      ...
      v13: {
        ...
        curatorHandoff: "只有用户明确选择的 2–6 件展品可作为未保存简报交给确定性策展工作流；7–20 件必须重新选择，系统不会静默截断、自动运行、保存或发布"
      },
      v14: {
      ...
      v171: {
        demoWriteBoundary: "公开 Demo 的 POST、PUT、PATCH、DELETE 在读取正文前统一拒绝，只读展示虚构播种数据与固定问题",
        trustContract: "四个主视图外持续展示模式、保存位置、AI 引擎、外发状态和静态加密状态；/api/runtime/trust 不返回 Key 或完整认证 URL",
        optionalOrganization: "先保存原文和同一件展品 ID，附件随后关联；整理预览零持久化，人工确认后用并发条件更新同一件展品",
        mobileInformationArchitecture: "底栏显示馆藏、记录、找回、我的；高级工程证据收进项目幕后，内部 hash 保持兼容"
      },
      v172: {
        persistentIndex: "用户明确选择后，浏览器只提交展品 ID、来源哈希和 512 维 Float32LE 派生向量到本机 SQLite；正文、照片和声音不随此写入请求重复上传。来源文字变化时旧向量失效；备份、导出、分享、公开 Demo 都不携带该缓存。",
        hybridRecall: "私人本地模式把字段与线索 Top 30 和新鲜本机向量 Top 30 用固定 RRF 合并，最多显示 20 件，并标注文字命中、意思相近或两者都命中。缓存缺失、过期或不可用时只回退字段检索。",
        sqliteVecBoundary: "sqlite-vec 只允许作为可删除的兼容性和性能实验；正式运行始终以可移植 SQLite BLOB 为真实来源，公开部署不加载本机 LLM 或持久向量索引。"
      },
      demo: demoStatus()
    };
  }
