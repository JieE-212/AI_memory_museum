...
function createMediaStorage(options = {}) {
  ...
  async function verifyVariant(variant, options = {}) {
    ...
    try {
      ...
      signal?.throwIfAborted?.();
      if (createHash("sha256").update(bytes).digest("hex") !== variant.sha256) return false;
      const kind = String(variant.kind || "");
      const info = kind === "display" || kind === "thumb"
        ? validateDerivedWebp(bytes, {
            maxBytes: policy.maxDerivedBytes,
            maxPixels: policy.maxPixelCount,
            expectedWidth: Number(variant.width),
            expectedHeight: Number(variant.height),
            maxWidth: kind === "thumb" ? policy.thumbMaxEdge : policy.displayMaxEdge,
            maxHeight: kind === "thumb" ? policy.thumbMaxEdge : policy.displayMaxEdge
          })
        : inspectImage(bytes, {
            maxBytes: policy.maxOriginalBytes,
            maxPixels: policy.maxPixelCount,
      ...
      return info.mimeType === variant.mimeType && info.width === Number(variant.width) && info.height === Number(variant.height);
    } catch (error) {
