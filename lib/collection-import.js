...
function createCollectionImporter({
  ...
  return function importCollection(body) {
    ...
    const featureBackups = [backupArchaeology, backupExhibitions, backupRevisits, backupEntities, backupVoices, backupCapsules];
    ...
    validateFeatureBackup({
      backup: backupVoices,
      validate: (value) => store.validateVoiceBackup(value, sourceMemoryIds),
      label: "声音",
      httpError
    });
    validateFeatureBackup({
    ...
    try {
      ...
      store.withTransaction(() => {
        result = store.importMemories(normalized, { clueMode: envelope.clueMode });
...
function assertEntityEnvelope(envelope, backup, httpError) {
  ...
  if (envelope.mode === "full" && backup === undefined) {
...
function emptyArchaeologyResult() {
  return { events: 0, claims: 0, decisions: 0, questions: 0, skipped: 0 };
}
