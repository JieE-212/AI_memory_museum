...
const SOURCE_TYPES = new Set(["memory-current", "revision", "raw-claim", "exif"]);
...
function initializeTimeCalibrationDatabase(options = {}) {
  ...
  function restoreTimeCalibrationBackup(backup, restoreOptions = {}) {
    ...
    return runAtomic(() => {
      ...
      for (const item of backup.calibrations) {
        ...
        const restoreSources = buildRestoreSourceMapping(workspace.candidates, memoryIdMap);
  ...
  function collectEvidence(memoryIds) {
    if (!memoryIds.length) return { memories: [], revisions: [], claims: [], observations: [] };
    ...
    return { memories, revisions, claims, observations };
  }
...
function buildRestoreSourceMapping(candidates, memoryIdMap) {
  ...
  for (const candidate of candidates) {
    const sourceMemoryId = sourceIdByTargetId.get(candidate.memoryId);
    if (!sourceMemoryId) {
      complete = false;
      continue;
    }
    const archiveCandidate = { ...candidate, memoryId: sourceMemoryId, sourceKey: "" };
