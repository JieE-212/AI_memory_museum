...
function buildTimeCandidates(input = {}) {
  ...
  const oralHistories = Array.isArray(input.oralHistories) ? input.oralHistories : [];
  ...
  for (const oral of oralHistories) {
    if (!isPlainObject(oral) || oral.sourceType !== "oral-history" || oral.status !== "confirmed") continue;
    const eventId = localId(oral.eventId);
    const interval = normalizeCalendarEvidence({
      resolutionKind: oral.precision || oral.resolutionKind,
      intervalStart: oral.intervalStart,
      intervalEnd: oral.intervalEnd
    });
    const assetContentSha256 = optionalSha256(oral.assetContentSha256);
    const transcriptSha256 = optionalSha256(oral.transcriptSha256);
    const segmentStartMs = optionalNonNegativeInteger(oral.segmentStartMs);
    const segmentEndMs = optionalNonNegativeInteger(oral.segmentEndMs);
    const questionKey = String(oral.questionKey || "");
    if (!eventId || !isSingleIntervalEvidence(interval) || !assetContentSha256 || !transcriptSha256 ||
        segmentStartMs === null || segmentEndMs === null || segmentStartMs >= segmentEndMs ||
        !/^oral-question:[a-f0-9]{64}$/u.test(questionKey)) continue;
    candidates.push(makeCandidate({
      sourceType: "oral-history",
      status: "confirmed",
      interval,
      eventId,
      eventTitle: displayText(oral.eventTitle, 120),
      questionKey,
      assetContentSha256,
      segmentStartMs,
      segmentEndMs,
      transcriptSha256,
      transcriptExcerpt: displayText(oral.transcriptExcerpt, 180)
    }));
  }
...
function buildStableSourceKey(candidate) {
  ...
  const eventId = localId(firstDefined(candidate.eventId, candidate.event_id));
  if (!sourceType || !status || !precision || !parseCanonicalDay(intervalStart) ||
  ...
  if (sourceType === "oral-history") {
    const questionKey = String(candidate.questionKey || "");
    const transcriptSha256 = optionalSha256(candidate.transcriptSha256);
    const segmentStartMs = optionalNonNegativeInteger(candidate.segmentStartMs);
    const segmentEndMs = optionalNonNegativeInteger(candidate.segmentEndMs);
    if (!eventId || memoryId || status !== "confirmed" || !/^oral-question:[a-f0-9]{64}$/u.test(questionKey) ||
        !transcriptSha256 || segmentStartMs === null || segmentEndMs === null || segmentStartMs >= segmentEndMs) {
      throw calibrationError("Oral-history time source is incomplete.", "CALIBRATION_SOURCE_INVALID");
    }
    projection.eventIdentitySha256 = sha256(`time-calibration-event\u0000${eventId}`);
    projection.questionKey = questionKey;
    projection.segmentEndMs = segmentEndMs;
    projection.segmentStartMs = segmentStartMs;
    projection.transcriptSha256 = transcriptSha256;
  } else {
    if (!memoryId || eventId) {
      throw calibrationError("Time source candidate has an invalid evidence anchor.", "CALIBRATION_SOURCE_INVALID");
    }
    projection.memoryIdentitySha256 = sha256(`time-calibration-memory\u0000${memoryId}`);
  }
...
function makeCandidate(input) {
  ...
  for (const key of [
    ...
    "questionKey", "segmentStartMs", "segmentEndMs", "transcriptSha256", "transcriptExcerpt"
  ]) {
...
function compareCandidates(left, right) {
  return compareText(left.sourceKey, right.sourceKey) || compareText(left.memoryId, right.memoryId) || compareText(left.eventId, right.eventId);
}
