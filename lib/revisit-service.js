...
function selectRevisits(inputMemories, inputStates, options = {}) {
  ...
  const offset = context.cursorOffset;
  const page = withIntents.items.slice(offset, offset + context.limit);
  return {
    kind: context.kind,
    ...
    nextCursor: offset + page.length < withIntents.items.length
      ? encodeRevisitCursor(context, offset + page.length)
      : null,
...
function normalizeRevisitContext(options = {}) {
  ...
  return {
    ...
    cursorOffset: decodeRevisitCursor(options.cursor, { ...local, kind })
  };
}

function encodeRevisitCursor(context, offset) {
...
function decodeRevisitCursor(value, context) {
  if (!value) return 0;
  try {
    const payload = JSON.parse(Buffer.from(String(value), "base64url").toString("utf8"));
    if (payload?.v !== 1 || payload.kind !== context.kind || payload.localDate !== context.localDate || payload.timezone !== context.timezone) {
      throw new Error("cursor context mismatch");
    }
    if (!Number.isSafeInteger(payload.offset) || payload.offset < 0) throw new Error("cursor offset invalid");
    return payload.offset;
  } catch {
    throw revisitError("cursor 无效、已过期或不属于当前回访方式。", "REVISIT_CURSOR_INVALID");
  }
}
