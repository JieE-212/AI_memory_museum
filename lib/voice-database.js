...
function initializeVoiceDatabase(options = {}) {
  ...
  const onConfirmedTranscriptChanged = typeof options.onConfirmedTranscriptChanged === "function"
    ...
    : null;
  const getAdditionalAssetUsage = typeof options.getAdditionalAssetUsage === "function"
    ? options.getAdditionalAssetUsage
    : () => 0;
  ...
  function listVoiceAssets(filters = {}) {
    ...
    return statements.listAssetsWithUsage.all()
      ...
      .filter((row) => !unreferenced || (Number(row.reference_count) || 0) + additionalUsage(row.id) === 0)
      .slice(offset, offset + limit)
      .map((row) => ({
        ...rowToAsset(row),
        referenceCount: (Number(row.reference_count) || 0) + additionalUsage(row.id)
      }));
  }
  ...
  function updateVoiceAsset(assetId, patch = {}) {
    ...
    if (status === "pending_delete" && totalAssetUsage(id) > 0) {
  ...
  function getVoiceUsage(assetId) {
    ...
    const oralHistoryCount = additionalUsage(id);
    return {
      assetId: id,
      status: asset.status,
      ...
      memoryIds,
      oralHistoryCount,
      referenceCount: memoryIds.length + oralHistoryCount,
  ...
  function exportVoiceData(mode = "full", sourceMemoryIds, exportOptions = {}) {
    assertPlainObject(exportOptions, "voice export options");
    assertKnownKeys(exportOptions, new Set(["additionalAssetIds"]), "voice export options");
    ...
    const additionalAssetIds = exportOptions.additionalAssetIds === undefined
      ? []
      : normalizeSourceAssetIds(exportOptions.additionalAssetIds);
    const assetIds = new Set([...links.map((row) => row.asset_id), ...additionalAssetIds]);
  ...
  function validateVoiceData(data, sourceMemoryIds, validationOptions = {}) {
    normalizeVoiceBackup(data, sourceMemoryIds, validationOptions);
    return true;
  }
  ...
  function restoreVoiceData(data, restoreOptions = {}) {
    ...
    assertKnownKeys(optionsObject, new Set([
      "memoryIdMap", "assetIdMap", "storageKeyMap", "additionalAssetIds", "notifyConfirmedTranscriptChanges"
    ]), "restore options");
    if (optionsObject.notifyConfirmedTranscriptChanges !== undefined &&
        typeof optionsObject.notifyConfirmedTranscriptChanges !== "boolean") {
      throw voiceError(
        "notifyConfirmedTranscriptChanges must be a boolean.",
        "VOICE_BACKUP_MAPPING_INVALID",
        400
      );
    }
    const shouldNotifyConfirmedTranscriptChanges = optionsObject.notifyConfirmedTranscriptChanges !== false;
    ...
    const additionalAssetIds = optionsObject.additionalAssetIds === undefined
      ? []
      : normalizeSourceAssetIds(optionsObject.additionalAssetIds);
    const normalized = normalizeVoiceBackup(data, sourceBoundary, { additionalAssetIds });
    ...
    return runAtomic(() => {
      ...
      for (const memoryId of linksByTargetMemory.keys()) {
        if (shouldNotifyConfirmedTranscriptChanges && confirmedMemories.has(memoryId)) {
          notifyConfirmedTranscriptChanged(memoryId, {
            assetId: null,
            reason: "restore",
            previousTranscript: null,
  ...
  function additionalUsage(assetId) {
    const value = getAdditionalAssetUsage(assetId);
    if (value && typeof value.then === "function") throw new TypeError("声音附加引用计数必须同步返回。");
    const count = Number(value);
    if (!Number.isSafeInteger(count) || count < 0) throw new TypeError("声音附加引用计数无效。");
    return count;
  }

  function totalAssetUsage(assetId) {
    return (Number(statements.countLinksForAsset.get(assetId)?.count) || 0) + additionalUsage(assetId);
  }
...
function normalizeVoiceBackup(data, sourceMemoryIds, options = {}) {
  ...
  assertPlainObject(options, "voice backup validation options");
  assertKnownKeys(options, new Set(["additionalAssetIds"]), "voice backup validation options");
  const additionalAssetIds = options.additionalAssetIds === undefined
    ? []
    : normalizeSourceAssetIds(options.additionalAssetIds);
  const usedAssetIds = new Set([...memoryLinks.map((item) => item.assetId), ...additionalAssetIds]);
  if (additionalAssetIds.some((assetId) => !assetById.has(assetId))) {
    throw voiceError("口述史引用了声音备份中缺失的资产。", "VOICE_BACKUP_REFERENCE_INVALID", 400);
  }
...
function normalizeSourceAssetIds(value) {
  if (!Array.isArray(value) || value.length > MAX_BACKUP_ASSETS) {
    throw voiceError("additionalAssetIds 数量无效。", "VOICE_BACKUP_INVALID", 400);
  }
  const ids = value.map((item, index) => requireId(item, `additionalAssetIds[${index}]`));
  assertUnique(ids, "additionalAssetIds", "VOICE_BACKUP_DUPLICATE");
  return ids;
}
