...
const CURATOR_AGENT_LIMITS = Object.freeze({
  // A retained run is independently capped at 512 KiB / 12,000 JSON nodes.
  // Twelve such runs stay below the section-wide 8 MiB text, 20 MiB JSON,
  // and 200,000-node restore budgets even at their individual maxima.
  runs: 12,
  stepsPerRun: 6,
  decisionsPerRun: 3,
  proposalsPerRun: 1,
  totalItems: 132,
  runJsonBytes: 512 * 1024,
  runJsonNodes: 12_000,
...
// Older V10 run archives may contain bounded internal planning receipts.
// They remain valid for restore compatibility, but only the fixed read-only
// tools count toward the persisted tool-call budget.
...
function validateRunEntry(entry, index, ids, boundaries) {
  ...
  const measured = measureJson(entry);
  if (measured.bytes > CURATOR_AGENT_LIMITS.runJsonBytes ||
      measured.nodes > CURATOR_AGENT_LIMITS.runJsonNodes) {
    fail(`${label} 超过单次运行的安全归档上限。`, "CURATOR_AGENT_ARCHIVE_LIMIT_EXCEEDED", 413);
  }
  ...
  for (let decisionIndex = 1; decisionIndex < normalizedDecisions.length; decisionIndex += 1) {
    if (compareDecisionOrder(normalizedDecisions[decisionIndex - 1], normalizedDecisions[decisionIndex]) > 0) {
      fail("人工决定必须按规范时间与动作依赖稳定排序。", "CURATOR_AGENT_DECISION_STATE_INVALID");
    }
  }
...
function validateRun(value, label, ids, boundaries) {
  ...
  if (Date.parse(updatedAt) < Date.parse(createdAt) ||
      (startedAt && (Date.parse(startedAt) < Date.parse(createdAt) || Date.parse(startedAt) > Date.parse(updatedAt)))) {
  ...
  for (const [terminalStatus, timestamp] of Object.entries(terminalTimes)) {
    ...
    const terminalFloor = startedAt || createdAt;
    if (timestamp && (Date.parse(timestamp) < Date.parse(terminalFloor) || Date.parse(timestamp) > Date.parse(updatedAt))) {
  ...
  return {
    ...
    completedAt, cancelledAt, interruptedAt, failedAt
  };
}
...
function validateStep(value, label, run, expectedPosition, ids, boundaries) {
  ...
  const createdAt = requireTimestamp(value.createdAt, `${label}.createdAt`);
  const stepFloor = run.startedAt || run.createdAt;
  const stepCeiling = run.completedAt || run.failedAt || run.cancelledAt || run.interruptedAt || run.updatedAt;
  if (Date.parse(createdAt) < Date.parse(stepFloor) || Date.parse(createdAt) > Date.parse(stepCeiling)) {
...
function validateProposal(value, label, run, steps, ids, boundaries) {
  ...
  const createdAt = requireTimestamp(value.createdAt, `${label}.createdAt`);
  if (!run.completedAt || Date.parse(createdAt) < Date.parse(run.startedAt || run.createdAt) ||
      Date.parse(createdAt) > Date.parse(run.completedAt)) {
    fail(`${label}.createdAt 超出运行边界。`, "CURATOR_AGENT_TIMESTAMP_INVALID");
  }
  return value;
}
...
function validateDecision(value, label, run, proposal, index, ids, boundaries) {
  ...
  const createdAt = requireTimestamp(value.createdAt, `${label}.createdAt`);
  const decisionFloor = proposal?.createdAt && run.completedAt
    ? Math.max(Date.parse(proposal.createdAt), Date.parse(run.completedAt))
    : NaN;
  if (!Number.isFinite(decisionFloor) || Date.parse(createdAt) < decisionFloor || Date.parse(createdAt) > Date.parse(run.updatedAt)) {
    fail(`${label}.createdAt 超出运行边界。`, "CURATOR_AGENT_TIMESTAMP_INVALID");
  }
  ...
  return { index, action, decision, createdAt };
}

function compareDecisionOrder(left, right) {
  const byTime = Date.parse(left.createdAt) - Date.parse(right.createdAt);
  if (byTime) return byTime;
  return CURATOR_AGENT_ACTIONS.indexOf(left.action) - CURATOR_AGENT_ACTIONS.indexOf(right.action);
}
