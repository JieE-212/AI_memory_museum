...
function buildSemanticRecallSnapshot(input = {}) {
  ...
  const documents = memories.map((memory) => {
    ...
    return buildSemanticRecallDocument(memory, voices, memoryId);
...
function buildSemanticRecallDocument(memory, voices, suppliedMemoryId) {
  const confirmedTranscripts = Array.isArray(voices) && voices.length
    ? readConfirmedTranscripts(voices)
    : projectConfirmedTranscripts(Array.isArray(memory?.confirmedTranscripts) ? memory.confirmedTranscripts : []);
  const document = {
    memoryId: suppliedMemoryId || normalizeMemoryId(memory?.id),
    ...
    confirmedTranscripts: Object.freeze(confirmedTranscripts)
  };
  return Object.freeze({ ...document, sourceSha256: semanticDocumentSourceSha256(document) });
}

function semanticDocumentSourceSha256(document) {
  return createHash("sha256").update(canonicalJson({
    title: document?.title || "",
    exhibitText: document?.exhibitText || "",
    rawContent: document?.rawContent || "",
    tags: Array.isArray(document?.tags) ? document.tags : [],
    confirmedTranscripts: Array.isArray(document?.confirmedTranscripts) ? document.confirmedTranscripts : []
  }), "utf8").digest("hex");
}
...
function readConfirmedTranscripts(voices) {
  ...
  return projectConfirmedTranscripts(texts);
}

function projectConfirmedTranscripts(value) {
  return fitTextList(Array.isArray(value) ? value : [], {
