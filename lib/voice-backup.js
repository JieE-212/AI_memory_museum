...
function buildVoiceArchiveSection({ collection, mode, schemaVersion, voiceStorage, validateVoiceBackup, streamFiles = false } = {}) {
  ...
  section.audioEntries = state.assets.map((asset) => readVoiceAssetForArchive(asset, voiceStorage, streamFiles));
  section.totalBytes = section.audioEntries.reduce((sum, entry) => safeAdd(sum, entry.byteSize), 0);
  return section;
}
...
function readVoiceAssetForArchive(asset, voiceStorage, streamFiles = false) {
