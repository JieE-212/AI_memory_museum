...
function executeCuratorAgent(options = {}) {
  ...
  function checkpoint() {
    ...
    const elapsed = Math.max(0, Number(monotonicNow()) - startedAtMs);
  ...
  function callTool(toolName, args, summary) {
    ...
    const callStarted = Number(monotonicNow());
    ...
    const receipt = Object.freeze({
      ...
      resultBytes: byteLength,
      durationMs: Math.max(0, Number(monotonicNow()) - callStarted),
  ...
  const elapsedMs = Math.max(0, Number(monotonicNow()) - startedAtMs);
  if (elapsedMs > budgets.maxDurationMs) {
    throw curatorAgentError("The run exceeded its duration budget.", "CURATOR_AGENT_DURATION_BUDGET_EXCEEDED", 409);
  }
  return Object.freeze({
    proposal,
...
function evaluateCuratorAgentTrace(snapshot = {}) {
  ...
  check("tool-budget", steps.length <= budgets.maxToolCalls, `${steps.length}/${budgets.maxToolCalls} read-only tool receipts.`);
  check("tool-whitelist", steps.every((step) => READ_ONLY_TOOL_SET.has(String(step.toolName || ""))), "Every receipt uses the fixed read-only whitelist.");
