...
- 2026-07-20 用户曾确认 V14 在手机 Wi-Fi 与蜂窝网络下入口可达；该历史结果不继承为 V17 真机验收。V17 手机真机、缩零冷启动与临时写入消失仍应继续单独复核。
...
CloudBase 地址继续作为简历国内主入口，Vercel V17 保留为全球备用入口。当前 V17 已完成桌面 UI、接口与模型资产发布闭环，但手机 Wi-Fi 与手机蜂窝网络的真机复核尚未完成，因此尚未满足覆盖电脑、手机 Wi-Fi 和手机蜂窝网络的完整验收门槛；只有三端复核全部通过，才能把它更新到简历并写成“V17 手机验收通过”。缩零后的首次冷启动与临时内容消失仍是独立运维待办，完成前不得声称恢复场景已经验收。
