...
- 用户已确认手机 Wi-Fi 与蜂窝网络均可打开国内入口，移动公网可达验收通过。该结果只证明两类移动网络下入口可达；缩零后的冷启动与临时写入消失仍应继续单独复核。
...
发布门槛仍是：只有完成电脑、手机 Wi-Fi 和手机蜂窝网络访问，才能把它更新到简历。当前桌面公网首页与四个 API 已通过，手机 Wi-Fi 与蜂窝网络入口可达也已由用户真机确认，因此本轮已满足该门槛，CloudBase 地址现在可以作为最终简历主入口；现有 Vercel V14 地址保留为全球备用入口。缩零后的首次冷启动与临时内容消失仍是明确的独立运维待办，完成前不得声称恢复场景已经验收。
