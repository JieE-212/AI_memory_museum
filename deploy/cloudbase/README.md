...
当前免费体验环境每月提供 3000 资源点，控制台“套餐用量”页会分别记录云托管 CPU、内存与外网出流量；“按量付费”必须始终保持关闭。本项目选择 0.5 核 / 1 GiB，计算消耗约为 59.5 点/实例小时，3000 点在不计流量时约覆盖 50 小时实例存活时间。实例闲置约 10 分钟后缩到 0，因此少量面试访问通常够用，但这不是无限免费承诺：若平台要求升级套餐、开启超限按量或创建常驻实例，立即停止，不继续发布。
...
- `/api/version` 返回当前发布版本；V14 基线应为 `14.0.0`、`schemaVersion: 19`。
...
CloudBase 地址通过上述验收后，才能把它更新到简历。现有 Vercel V14 地址在此之前继续作为备用入口，不要提前宣称 CloudBase 已发布。
