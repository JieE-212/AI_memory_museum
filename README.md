...
>
> CloudBase 国内入口状态（2026-07-20）：服务 `time-isle-demo` 的部署 `001` 从提交 `278f925` 构建并发布；桌面公网已验收首页、`/api/version`、`/api/health`、`/api/demo/status` 与 `/api/memories`，确认 `V14.0.0 / schema 19 / 4` 条播种记忆。运行规格为 `0.5` 核 / `1 GiB`、最小 `0` / 最大 `1` 个实例，SQLite 与媒体只写 `/tmp`；按量付费关闭，截至 2026-07-20 桌面验收时控制台显示套餐用量为 `0.11 / 3000` 点。手机 Wi-Fi 与蜂窝网络真机访问仍待用户验收，不能表述为已完成移动网络验收。
>
...
V14.0.0 / schema 19 已于 2026-07-20 完成双远端发布与 Vercel 生产核验；CloudBase Run 国内入口已完成桌面公网首页和四 API 验收，手机 Wi-Fi、蜂窝网络与缩零冷启动仍待验收。V10.0.0 / schema 14 保留为 2026-07-19 的历史发布基线。更多说明见 [产品说明](./项目文档/产品说明.md)、[技术设计](./项目文档/技术设计.md) 和 [V5–V10 扩展路线](./项目文档/V5-V10扩展路线.md)。部署步骤见 [DEPLOYMENT.md](./DEPLOYMENT.md)。
