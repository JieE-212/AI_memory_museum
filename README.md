...
> 本地版本状态（2026-07-18）：V9.0.0（schema 13）已完成实现与最终验收，`npm.cmd run check` 全绿；真实 HTTP smoke 227 条，桌面、390px 与 320px 真实浏览器视口均通过，窄屏无横向溢出、关键触控目标达到 44px、控制台零错误。当前尚未推送、尚未部署，也不提前把本地提交写成已完成事实。上一版 V8.0.0（schema 12）的 211 条 smoke、三档浏览器验收与独立审计继续作为历史事实保留。
>
...
本地 V9 的公开 Demo 配置沿用 4 件示例展品、1 场可分享的已确认展览，以及由两段校园记录组成的 1 个已确认事件和 1 项“保留多种记录”时间校准；符合条件时还可只读展示口述史问题与既有来源，但不会伪造可播放回答。Demo 不请求麦克风、不打开音频文件选择，也不开放口述史、展览、回访意愿、校准、胶囊或媒体持久化。以上变化尚未进入线上 V7.1。

## 60 秒看懂

1. 在公开 Demo 的展品库搜索“阿棠”，查看两条毕业记忆及其命中字段、确认实体与短词回退依据。
2. 打开《操场尽头的告别》，沿这段记忆漫游到《后来写下的毕业傍晚》；进入时光拼图，对照双侧原文锚点和一天的日期差异。
3. 在本地 V9 的时光拼图展开“来源校准台”和“一问一答口述史”：先核对书面日期来源；仍无法解释冲突时，再由用户录音或选择音频、手工划定片段、填写文字稿并明确时间含义。两条链路都不会改写两段原文和展品日期。
...
flowchart TB
  ...
  H --> D["记忆 · 修订 · 媒体 · 策展 · 口述史 · 校准 · 胶囊 · 归档"]
  D --> DB[("SQLite")]
  D --> FS[("内容寻址媒体目录")]
  D --> G["人工口述证据 · 来源校准 · 明确回访意愿 · 三层分享核对"]
  G --> C["浏览器 Web Crypto"]
  C --> O["口令加密、断网可读 HTML"]
...
页面始终只有“展品库、记录记忆、讲解与回顾、数据与项目”四项顶层导航。主题展览、回访、线索检索、人物档案、语音、口述史、胶囊、记忆年轮、不确定时间线和馆藏体检均放在已有任务流内，通过默认折叠层、对话层或独立阅读层渐进呈现；移动端继续优先保证单列阅读与固定四项底栏。
...
- 完整 `.time-isle` 归档包含馆藏、照片与声音二进制、媒体关联、图片线索、时光拼图、主题展览、回访状态与明确意愿、人物档案、事件级口述史、时间校准、时间胶囊和记忆修订；Agent 运行日志当前不在归档内。口述史结构位于 `oral-history/state.json`，声音字节仍只存一份。
...
npm.cmd run smoke   # 227 条本地真实 HTTP 端到端断言
...
- 当前适合个人本地使用和面试演示；多用户认证、跨设备同步、云端持久数据库与云媒体存储不在 V9.0.0 范围内。

更多说明见 [产品说明](./项目文档/产品说明.md)、[技术设计](./项目文档/技术设计.md) 和 [V5–V9 扩展路线](./项目文档/V5-V9扩展路线.md)。部署步骤见 [DEPLOYMENT.md](./DEPLOYMENT.md)。
