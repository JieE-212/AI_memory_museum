...
- Live Demo（V14.0.0）: https://ai-memory-museum-demo.vercel.app
...
>
> 线上发布状态：公开 Demo 已部署 V14.0.0（schema 19）。2026-07-20 已核验 `/api/version` 与 `/api/health` 返回 `version: 14.0.0`、`schemaVersion: 19`、`mode: interview-demo`、`storage: ephemeral-sqlite`、`aiMode: mock-fallback`；`GET /api/curator-agent/sample` 返回 `synthetic: true`、`demo: true`，持久化运行数保持 0。分别向锁馆和结构演练接口发送虚构 `text/plain` 探针，均在读取正文前返回 `403 / MUSEUM_LOCK_DEMO_READ_ONLY / bodyBytesRead: 0`，探针前后的完整 stats、锁状态与运行数均未变化。实际版本仍以线上接口返回值为准。
...
V14.0.0 / schema 19 已于 2026-07-20 完成双远端发布与 Vercel 生产核验；V10.0.0 / schema 14 保留为 2026-07-19 的历史发布基线。更多说明见 [产品说明](./项目文档/产品说明.md)、[技术设计](./项目文档/技术设计.md) 和 [V5–V10 扩展路线](./项目文档/V5-V10扩展路线.md)。部署步骤见 [DEPLOYMENT.md](./DEPLOYMENT.md)。
