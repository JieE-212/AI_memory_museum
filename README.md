# 时屿（TIME ISLE）V14.0.0

> 本地优先的 AI 私人记忆策展工具

把散落的日记、聊天片段、照片与声音整理成一座可检索、可回顾的私人记忆岛屿；再把用户确认过的展览封存到未来，或生成口令加密、断网可读的单文件。

- 国内 Live Demo（CloudBase，V14.0.0）: https://shiyu-memory-demo-d3di282387d5c7-1456049152.ap-shanghai.app.tcloudbase.com
- 全球备用 Demo（Vercel，V14.0.0）: https://ai-memory-museum-demo.vercel.app
- GitHub（V14.0.0）: https://github.com/JieE-212/AI_memory_museum
- Gitee（V14.0.0）: https://gitee.com/JieE-212/ai-memory-museum
...
> 当前版本边界（2026-07-21）：上面的公开 Demo、GitHub 与 Gitee 仍是已发布的 **V14.0.0**。本地工作区已经完成 **V17.0.0 候选版**（schema 仍为 19）的实现与验收，新增一次性真实隔离恢复、多视角记忆对照和设备内“按意思找回”；本轮尚未提交、推送或部署，不能把这些能力说成已经上线。
>
...
>
> CloudBase 国内入口状态（2026-07-20）：服务 `time-isle-demo` 的部署 `001` 从提交 `278f925` 构建并发布；桌面公网已验收首页、`/api/version`、`/api/health`、`/api/demo/status` 与 `/api/memories`，确认 `V14.0.0 / schema 19 / 4` 条播种记忆。运行规格为 `0.5` 核 / `1 GiB`、最小 `0` / 最大 `1` 个实例，SQLite 与媒体只写 `/tmp`；按量付费关闭，截至 2026-07-20 桌面验收时控制台显示套餐用量为 `0.11 / 3000` 点。用户随后确认手机 Wi-Fi 与蜂窝网络均可打开该入口，移动公网可达验收通过；缩零后的首次冷启动与临时写入消失仍待单独观察。
>
...
>
> Vercel 既有深度生产核验（2026-07-20）：`/api/version` 返回 `version: 14.0.0`；`/api/health` 返回 `schemaVersion: 19`、`mode: interview-demo`、`storage: ephemeral-sqlite`、`aiMode: mock-fallback`。`GET /api/curator-agent/sample` 返回 `synthetic: true`、`demo: true`，持久化运行数保持 0。分别向锁馆和结构演练接口发送虚构 `text/plain` 探针，均在读取正文前返回 `403 / MUSEUM_LOCK_DEMO_READ_ONLY / bodyBytesRead: 0`，探针前后的完整 stats、锁状态与运行数均未变化。CloudBase 本轮只声明上文已执行的桌面首页与四 API 核验，以及手机 Wi-Fi 与蜂窝网络的入口可达结果；缩零冷启动仍待观察，实际版本始终以对应入口的线上接口为准。

当前线上 V14 公开 Demo 以 4 件示例数据启动，并使用临时 SQLite。它禁止私人图片或声音上传、媒体修改、展览与胶囊持久化、归档恢复、导入、删除与清空；访客仍可在事务硬上限内体验临时纯文本、整理运行、时光拼图与补问，这些内容可能被同一实例的其他访客看到，因此请勿提交私人信息。Demo 在代码层强制使用本地 Mock，即使环境误配 `AI_API_KEY` 也不会调用外部模型。

线上 V14 的公开 Demo 配置沿用 4 件示例展品、1 场可分享的已确认展览，以及由两段校园记录组成的 1 个已确认事件和 1 项“保留多种记录”时间校准；符合条件时还可只读展示口述史问题与既有来源，但不会伪造可播放回答。受限策展助手由 `/api/curator-agent/sample` 合成只读工作区，不创建运行或接触持久化；设备内镜片只对合成或播种来源生成确定性只读预览。Demo 不请求麦克风、不打开音频文件选择，并在正文前阻止共忆保存、锁馆切换、结构演练上传以及口述史、展览、回访意愿、校准、胶囊或媒体持久化。

## 60 秒看懂

1. 在公开 Demo 的展品库搜索“阿棠”，查看两条毕业记忆及其命中字段、确认实体与短词回退依据。
2. 打开《操场尽头的告别》，沿这段记忆漫游到《后来写下的毕业傍晚》；进入时光拼图，对照双侧原文锚点和一天的日期差异。
3. 在本地可写 V14 环境的时光拼图展开“来源校准台”和“一问一答口述史”：先核对书面日期来源；仍无法解释冲突时，再由用户录音或选择音频、手工划定片段、填写文字稿并明确时间含义。两条链路都不会改写两段原文和展品日期。
...
继续体验 V14 完整本地能力时，可再用虚构数据生成一封共忆信笺、运行四种设备内镜片，并在“数据与项目”锁馆后执行结构恢复演练；三项都默认折叠，且必须同时说明“回信不是身份认证、锁馆不是静态加密、结构演练不是恢复成功”。

本地 V17 候选版还可继续演示三步：先把完整备份真正恢复到一次性副本并在体检后销毁；再从单件展品查看当前记录、编辑年轮、亲友回信与来源关系；最后主动加载约 47 MB 的同源中文模型，用自然描述在本页内存中“按意思找回”。三项都放回已有页面并默认折叠，不增加第五项导航。
...
flowchart TB
  ...
  H --> D["记忆 · 修订 · 媒体 · 共忆 · 镜片 · 受限策展 · 口述史 · 校准 · 胶囊 · 归档"]
  D --> DB[("SQLite")]
  D --> FS[("内容寻址媒体目录")]
...
npm.cmd run smoke   # V14 当前发布基线为 262 条；V10 历史基线为 249 条
...
本地 V17 候选版在 schema 19 上追加 V15 一次性真实隔离恢复、V16 多视角只读对照和 V17 设备内中文语义回忆。2026-07-21，`npm.cmd run build`、`npm.cmd run check`、272 条真实 HTTP smoke 与 21/21 项 Playwright 三档门禁均已通过；桌面、390×844、320×700 的真实浏览器验收覆盖四导航、默认折叠、固定移动底栏、长文本和无横向溢出。真实 q8 ONNX/WASM 推理已成功，专项门禁确认无第三方请求，准备与释放前后不新增 localStorage、sessionStorage、IndexedDB 或 Cache Storage。以上是本地候选结果，不是公开 V14 的生产核验。
...
- 当前适合个人本地使用和面试演示；多用户认证、跨设备同步、云端持久数据库与云媒体存储不在 V14.0.0 范围内。

V14.0.0 / schema 19 已于 2026-07-20 完成双远端发布与 Vercel 生产核验；CloudBase Run 国内入口已完成桌面公网首页和四 API 验收，手机 Wi-Fi 与蜂窝网络入口可达也已由用户真机确认，因此作为国内主入口与最终简历入口，Vercel 保留为全球备用。缩零冷启动仍待单独观察。V17.0.0 候选版已在本地完成 V15–V17 实现与验收，但尚未提交、推送或发布；V18 停在总体 NO-GO 的可行性门禁，没有实现真正加密。更多说明见 [产品说明](./项目文档/产品说明.md)、[技术设计](./项目文档/技术设计.md)、[V15–V18 扩展路线](./项目文档/V15-V18扩展路线.md) 和 [V18.0 加密可行性门禁记录](./项目文档/V18.0加密可行性门禁记录.md)。部署步骤见 [DEPLOYMENT.md](./DEPLOYMENT.md)。
