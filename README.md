...
> 版本状态（2026-07-17）：V7.1.0（schema 9）已经完成实现、完整检查、迁移副本核对、本地验收、双远端推送与 Vercel 部署后验收；公开 Demo 的 `/api/version` 与 `/api/health` 已确认返回 V7.1.0 / schema 9。
...
3. 在本地 V7 从已确认展览进入“胶囊与分享”，展示未到期外壳、最小化素材选择和浏览器端口令加密。
...
页面始终只有“展品库、记录记忆、讲解与回顾、数据与项目”四项顶层导航。主题展览、回访、线索检索、人物档案、语音和胶囊均放在已有任务流内，通过折叠层、对话层或独立阅读层渐进呈现。
...
- 完整 `.time-isle` 归档包含馆藏、照片与声音二进制、媒体关联、图片线索、时光拼图，以及 V5–V7 的主题展览、回访、线索、人物档案和时间胶囊；Agent 运行日志当前不在归档内。
- 脱敏 `.time-isle` 会物理排除照片与声音文件，并隐藏原始正文、人物、地点、媒体备注和胶囊私密字段，不只是把前端入口藏起来。
...
项目工程/
  ...
  scripts/
    ...
    offline-exhibit-check.js      # 真实安全快照、浏览器打包、加密与离线阅读页回归
  项目文档/
    产品说明.md
    技术设计.md
    V5-V7扩展路线.md
...
npm.cmd run smoke   # 本地真实 HTTP 端到端断言
npm.cmd run check   # 上述全部检查
```

`npm.cmd test` 等价于 `npm.cmd run check`。检查数据使用系统临时目录，并在结束时清理，不会写入正式馆藏。

真实 HTTP smoke 覆盖静态页面与安全头、DNS rebinding Host 拒绝、同源写入、Mock 整理、展品 CRUD、检索与讲解、图片和语音链路、主题展览、回访、语义线索、胶囊的未到期零泄漏与到期读取、离线安全素材筛选、`.time-isle` 完整恢复与损坏零写入，以及公开 Demo 的强制 Mock、容量上限和禁写边界。独立脚本继续覆盖格式与存储事务、迁移、备份恢复、前端结构、浏览器端加密信封和自包含阅读页。
...
- 当前适合个人本地使用和面试演示；多用户认证、跨设备同步、云端持久数据库与云媒体存储不在 V7.1.0 范围内。
