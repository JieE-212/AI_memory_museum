# 时屿（TIME ISLE）V7.3.0 发布前与 V7.1.0 线上 Demo 说明
...
本文保留 V7.1.0（schema 9）的线上发布配置与验收事实，并追加 V7.3 的发布前口径。V7.1 已于 2026-07-17 完成本地检查、双远端推送和 Vercel 部署后验收；线上 `/api/health` 已核验 `version: 7.1.0`、`schemaVersion: 9`、`mode: interview-demo`、`storage: ephemeral-sqlite` 和 `aiMode: mock-fallback`。实际部署状态始终以该接口返回值为准。

本地 V7.3.0（schema 11）已于 2026-07-18 完成实现、完整检查与本地提交，真实 HTTP smoke 共 202 条断言；尚未推送、尚未部署，因此不能把本地结果写成线上发布事实。只有完成推送、新部署变为 `Ready`，且线上接口返回 `7.3.0 / schema 11` 后，才能更新本段状态。
...
面向下一次公开 Demo 的本地 V7.3 代码已确定性播种 4 件示例展品和 1 场可分享的已确认展览，使访客无需写入即可进入三层分享隐私编辑台。该展览仍位于临时 SQLite；公开 Demo 继续禁止展览、回访意愿、胶囊和媒体持久化，并继续禁止输入任何私人数据。此变化尚未部署到当前线上 V7.1。
...
`npm run build` 会执行语法检查和各模块回归，但不会启动真实 HTTP smoke；V7.3 的 202 条 HTTP 断言应在推送前由 `npm.cmd run check` 完成。
...
- 若未来接入云数据库或对象存储，需要额外实现身份认证、租户隔离、访问控制、密钥管理和存储驱动；这些不在 V7.3.0 范围内。
...
默认本地路径分别为 `data/memory-museum.sqlite`、`data/media/` 和 `data/media/voice/`。V7.3 schema 11 完整 `.time-isle` 会包含馆藏、照片、声音、确认文字稿、图片线索、实体图、主题展览、回访状态与用户明确回访意愿、时间胶囊、时光拼图和可校验记忆修订链，但不包含 Agent 运行日志；脱敏归档会物理排除正文、媒体文件、文字稿、展览叙事、实体名称、胶囊内容，以及回访意愿的展品 ID、选择、日期、时区和修订快照、编辑备注、哈希、精确时间、内部 ID，只保留必要计数摘要。
...
- `smoke`：在系统临时目录启动本地服务，执行当前 202 条真实 HTTP 断言。
- `check`：依次执行全部语法、独立回归和 HTTP smoke；`npm.cmd test` 与它等价。

回归覆盖真实格式校验、安全展示图、精确去重、关联保护与 GC、EXIF 待确认/GPS 敏感、区域证据、声音、主题展览、实体线索、胶囊锁定、三层分享隐私编辑台、浏览器端加密、明确回访意愿、记忆修订、馆藏体检，以及 schema 11 `.time-isle` 全量验真、损坏零写入和事务恢复。

## V7.2 与 V7.3 发布前新增验收（仅本地已完成）
...
V7.3 当前关键回归计数：真实 HTTP smoke 202 条；明确回访意愿 80 条、其 UI 40 条；离线展览 174 条；胶囊 UI 142 条；JSON 馆藏导入 109 条；媒体归档备份 191 条；媒体归档恢复 118 条；前端 96 项；PWA 79 条。

本地提交已在完整检查后形成；决定正式发布后才执行推送。GitHub 推送命令为：
...
V7.3 已形成本地提交，但尚未推送或部署。待新部署变为 `Ready` 后，必须重新确认 `/api/version` 与 `/api/health` 分别返回 `7.3.0` 和 schema 11，`/api/demo/status` 返回 `seededExamples: 4` 与 `seededExhibitions: 1`；同时验证修订恢复、体检启动或取消、回访意愿写入、展览与胶囊持久化、媒体写入、导入和归档验真在公开 Demo 中继续返回 403。完成这些检查前不得把线上版本标为 V7.3。

最后在无痕窗口完成一次人工路径：

1. 浏览四件示例展品，打开《操场尽头的告别》，进入记忆航线与时光拼图并核对原文摘录。
2. 体验 Mock AI 整理、语义线索检索、实体档案预览和讲解来源，不输入任何私人文本。
3. 体验主题展览预览、今日回访、明确回访意愿界面与声音只读提示；确认展览保存、回访状态与意愿、声音和胶囊写入均被 Demo 阻止。
4. 从播种的已确认展览打开“胶囊与分享”，确认页面明确提示公开 Demo 不保存胶囊；逐层核对公开外壳、口令内内容与固定排除项，仅使用示例内容体验浏览器内加密，不复用私人口令。
...
完整闭环的最终验收应在本地使用可丢弃、无隐私的数据完成：连续编辑一件展品并验证冲突保护、no-op 和旧版追加式恢复，运行只读馆藏体检，以只读方式验真完整与脱敏 `.time-isle`，再把完整归档恢复到另一组临时 `DB_PATH` / `MEDIA_ROOT`；图片、声音、胶囊和离线口令链路仍按既有路径核对。测试后删除临时数据和下载文件，不把测试素材提交到仓库。
